...
export class TypeSenseController {
  ...
  @HasuraEvent()
  public async player_events(data: HasuraEventData<players_select_column>) {
    if (data.op === "DELETE") {
      await this.typeSense.removePlayer(data.old.steam_id);
      return;
    }

    await this.typeSense.updatePlayer(data.new.steam_id);
  }

  @HasuraEvent()
  public async team_roster_events(
    data: HasuraEventData<team_roster_select_column>
  ) {
    await this.typeSense.updatePlayer(
      data.new.player_steam_id || data.old.player_steam_id
    );
  }
}
