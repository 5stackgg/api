...
import { UtilityLibraryRow, UtilityLineupsService } from "./utility-lineups.service";
...
export type SaveUtilityPlaybookInput = {
...
export type UtilityPlaybookStep = {
  ...
  assigned_steam_id: string | null;
  note: string | null;
  lineup: UtilityLibraryRow;
};

export type UtilityPlaybookPayload = {
  id: string;
  name: string;
  map_name: string;
  side: string;
  steps: Array<UtilityPlaybookStep>;
};
...
@Injectable()
export class UtilityPlaybooksService {
  ...
  public async save(
    user: User,
    input: SaveUtilityPlaybookInput,
  ): Promise<{ id: string }> {
    const name = UtilityLineupsService.sanitizeName(input.name, "Playbook");
    const description = UtilityLineupsService.sanitizeText(
      input.description,
      1000,
    );
    ...
    const steps =
      input.steps === undefined || input.steps === null
        ? null
        : UtilityPlaybooksService.normalizeSteps(input.steps);

    return await this.postgres.transaction(async (client) => {
      let playbookId = input.playbook_id ?? null;

      if (playbookId) {
        if (!UtilityPlaybooksService.UUID.test(playbookId)) {
        ...
        const {
          ...
          `SELECT public.can_edit_utility_playbook(p, $2::json) AS editable
             FROM public.utility_playbooks p WHERE p.id = $1::uuid`,
        ...
        // Steps go first: tbiu_utility_playbooks refuses to move a written book
        // to another map, and a rewrite legitimately does both at once.
        if (steps) {
          await client.query(
            "DELETE FROM public.utility_playbook_steps WHERE playbook_id = $1::uuid",
            [playbookId],
          );
        }

        await client.query(
          `UPDATE public.utility_playbooks
              SET name = $2,
                  description = $3,
                  map_name = $4,
                  side = $5,
        ...
        const {
          ...
          `INSERT INTO public.utility_playbooks
  ...
  public async remove(
    ...
    if (!UtilityPlaybooksService.UUID.test(String(playbookId ?? ""))) {
      throw Error("playbook not found");
    }
    ...
    await this.postgres.query(
      "DELETE FROM public.utility_playbooks WHERE id = $1::uuid",
      [playbookId],
    );
  ...
  public async forSession(
    sessionId: string,
  ): Promise<UtilityPlaybookPayload | null> {
    ...
    >(
      `SELECT p.id::text AS id, p.name, p.map_name, p.side
         ...
         INNER JOIN public.utility_playbooks p ON p.id = s.playbook_id
        WHERE s.id = $1::uuid`,
      [sessionId],
    );
  ...
  public async steps(playbookId: string): Promise<Array<UtilityPlaybookStep>> {
    ...
    >(
      `SELECT st.step_order, st.offset_ms,
              st.utility_lineup_id::text AS utility_lineup_id,
              st.assigned_steam_id::text AS assigned_steam_id,
              st.note,
              l.id::text AS id, l.name, l.map_name, l.utility_type, l.side,
  ...
  private async insertSteps(
    ...
    const { rows: lineups } = await client.query<{
      ...
      [steps.map((step) => step.utility_lineup_id), this.sessionJson(user)],
    );

    const {
      rows: [playbook],
    } = await client.query<{ map_name: string }>(
      "SELECT map_name FROM public.utility_playbooks WHERE id = $1::uuid",
      [playbookId],
    );

    for (const step of steps) {
      const lineup = lineups.find((row) => row.id === step.utility_lineup_id);
    ...
    for (const [index, step] of steps.entries()) {
      await client.query(
        `INSERT INTO public.utility_playbook_steps
           (playbook_id, utility_lineup_id, step_order, offset_ms, assigned_steam_id, note)
         VALUES ($1::uuid, $2::uuid, $3::int, $4::int, $5::bigint, $6)`,
        [
          playbookId,
          step.utility_lineup_id,
          index,
          step.offset_ms,
  ...
  private static normalizeSteps(
    ...
    if (steps.length > UtilityPlaybooksService.MAX_STEPS) {
      throw Error("that playbook has too many steps");
    }

    return steps.map((step) => {
      const lineupId = String(step?.utility_lineup_id ?? "");

      if (!UtilityPlaybooksService.UUID.test(lineupId)) {
      ...
      if (
        ...
        offset < 0 ||
        offset > UtilityPlaybooksService.MAX_OFFSET_MS
      ) {
      ...
      return {
        ...
        note: UtilityLineupsService.sanitizeText(step?.note, 200),
      };
    });
  }
}
