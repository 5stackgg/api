...
export type UtilityPracticePlanOutput = {
  analysed: boolean;
  message: string | null;
  entries: Array<UtilityPracticePlanEntry>;
};
...
export type UtilityTeamUtilityOutput = {
  analysed: boolean;
  message: string | null;
  entries: Array<UtilityTeamUtilityEntry>;
};
...
//
// Both are aggregates by construction. utility_demo_throws names players and is
// admin-only for that reason, so nothing here ever returns a row out of it --
// only counts over it.
@Injectable()
export class UtilityInsightsService {
  ...
  // public.utility_lineup_difficulty, which is the single definition -- the
  ...
  private static readonly DIFFICULTY_ORDER = [
    UtilityInsightsService.EASY,
    UtilityInsightsService.MODERATE,
    UtilityInsightsService.HARD,
  ...
  // public.utility_verify_masteries defaults to for calling a lineup verified.
  ...
  // (utility_meta_lineups.throwers, mined out of real demos) and what the caller
  // has actually drilled (utility_lineup_progress).
  public async practicePlan(
  ...
  ): Promise<UtilityPracticePlanOutput> {
    ...
    const order = UtilityInsightsService.order(input.order);
    const side = UtilityInsightsService.text(input.side);
    const limit = UtilityInsightsService.clamp(
      input.limit,
      UtilityInsightsService.DEFAULT_PLAN_LIMIT,
      UtilityInsightsService.MAX_PLAN_LIMIT,
    );
    ...
    const [meta] = await this.postgres.query<Array<{ mined: boolean }>>(
      `SELECT EXISTS (
                SELECT 1 FROM public.utility_meta_lineups m WHERE m.map_name = $1
              ) AS mined`,
      [mapName],
    );
    ...
    const candidates = await this.postgres.query<Array<PlanCandidate>>(
      `SELECT DISTINCT ON (m.lineup_bucket)
              l.id::text AS utility_lineup_id,
         ...
         FROM public.utility_meta_lineups m
         INNER JOIN public.utility_lineups l
                 ON l.lineup_bucket = m.lineup_bucket
         LEFT JOIN public.utility_lineup_progress p
                ON p.utility_lineup_id = l.id
        ...
        ORDER BY m.lineup_bucket,
                 (p.utility_lineup_id IS NOT NULL) DESC,
                 (l.verified_at IS NOT NULL) DESC,
                 l.upvotes DESC,
                 l.created_at ASC
        LIMIT ${UtilityInsightsService.PLAN_CANDIDATES}`,
      [mapName, user.steam_id, side, UtilityInsightsService.session(user)],
    );

    const entries = candidates
      .map((candidate) => UtilityInsightsService.rank(candidate))
      .filter((entry): entry is UtilityPracticePlanEntry => entry !== null)
      .sort((a, b) => UtilityInsightsService.compare(a, b, order))
      .slice(0, limit);
  ...
  private static rank(candidate: PlanCandidate): UtilityPracticePlanEntry | null {
    ...
    const measured = candidate.difficulty !== UtilityInsightsService.UNMEASURED;

    return {
      utility_lineup_id: candidate.utility_lineup_id,
      priority: Math.round(throwers * gap * 1000) / 1000,
      meta_throwers: throwers,
      attempts,
      successes,
      mastered: candidate.mastered,
      reason: UtilityInsightsService.reason(candidate, throwers, attempts),
  ...
  private static compare(
    a: UtilityPracticePlanEntry,
    b: UtilityPracticePlanEntry,
    order: string,
  ): number {
    if (order !== UtilityInsightsService.ORDER_PRIORITY) {
      const band =
        UtilityInsightsService.band(a.difficulty) -
        UtilityInsightsService.band(b.difficulty);
      ...
      const rank =
        UtilityInsightsService.DIFFICULTY_ORDER.indexOf(a.difficulty) -
        UtilityInsightsService.DIFFICULTY_ORDER.indexOf(b.difficulty);

      if (rank !== 0) {
        return order === UtilityInsightsService.ORDER_QUICK_WINS ? rank : -rank;
      }
    }
    ...
    return (
      ...
      a.utility_lineup_id.localeCompare(b.utility_lineup_id)
    );
  }

  private static band(difficulty: string): number {
    return difficulty === UtilityInsightsService.UNMEASURED ? 1 : 0;
  }

  private static order(value: unknown): string {
    const order =
      UtilityInsightsService.text(value) ?? UtilityInsightsService.ORDER_PRIORITY;

    if (
      order !== UtilityInsightsService.ORDER_PRIORITY &&
  ...
  private static reason(
    ...
    if (candidate.mastered) {
      return UtilityInsightsService.MASTERED_SLIPPING;
    }

    if (attempts === 0) {
      return UtilityInsightsService.NEVER_ATTEMPTED;
    }

    if (throwers >= UtilityInsightsService.POPULAR_THROWERS) {
      return UtilityInsightsService.POPULAR_UNMASTERED;
    }

    return UtilityInsightsService.UNMASTERED;
  }
  ...
  // (UtilityLineupsService.decomposeMiss), so this is arithmetic over one row per
  ...
  // reason the sums live on utility_lineup_progress. Somebody who drills a lineup
  ...
  public async missPattern(
    ...
    if (!UtilityInsightsService.UUID.test(lineupId)) {
    ...
    const [lineup] = await this.postgres.query<Array<{ visible: boolean }>>(
      `SELECT public.can_view_utility_lineup(l, $2::json) AS visible
        ...
        WHERE l.id = $1::uuid`,
      [lineupId, UtilityInsightsService.session(user)],
    );
    ...
    const rows = await this.postgres.query<Array<MissSample>>(
      `SELECT p.miss_samples::int AS samples,
        ...
        WHERE p.utility_lineup_id = $1::uuid
    ...
    if (
      players < UtilityInsightsService.MIN_PATTERN_PLAYERS ||
      samples < UtilityInsightsService.MIN_PATTERN_SAMPLES
    ) {
      return {
        analysed: false,
        message: `not enough practice on this lineup to read a pattern yet -- it takes ${UtilityInsightsService.MIN_PATTERN_SAMPLES} throws by ${UtilityInsightsService.MIN_PATTERN_PLAYERS} players`,
        samples,
        players,
        mean_along: null,
        mean_lateral: null,
    ...
    return {
      ...
      samples,
      players,
      mean_along: UtilityInsightsService.round(UtilityInsightsService.mean(along)),
      mean_lateral: UtilityInsightsService.round(
        UtilityInsightsService.mean(lateral),
      ),
      mean_vertical: UtilityInsightsService.round(
        UtilityInsightsService.mean(vertical),
      ),
      bias: UtilityInsightsService.bias({ along, lateral, vertical }, radius),
    };
  }
  ...
  private static bias(
    ...
    const candidates = [
      {
        values: axes.along,
        low: UtilityInsightsService.BIAS_SHORT,
        high: UtilityInsightsService.BIAS_LONG,
      },
      {
        values: axes.lateral,
        low: UtilityInsightsService.BIAS_LEFT,
        high: UtilityInsightsService.BIAS_RIGHT,
      },
      {
        values: axes.vertical,
        low: UtilityInsightsService.BIAS_LOW,
        high: UtilityInsightsService.BIAS_HIGH,
      },
    ].map((axis) => ({
      ...axis,
      mean: UtilityInsightsService.mean(axis.values),
    }));
    ...
    if (
      ...
      radius * UtilityInsightsService.BIAS_FLOOR_RATIO
    ) {
      return UtilityInsightsService.BIAS_NONE;
    }
    ...
    if (
      Math.abs(dominant.mean) <
      UtilityInsightsService.spread(dominant.values) *
        UtilityInsightsService.BIAS_SIGNAL_RATIO
    ) {
      return UtilityInsightsService.BIAS_SCATTERED;
    }
  ...
  private static spread(values: Array<number>): number {
    const mean = UtilityInsightsService.mean(values);
  ...
  //
  // Gated on membership and aggregated in the database. utility_demo_throws is
  ...
  public async teamUtilityReport(
  ...
  ): Promise<UtilityTeamUtilityOutput> {
    const teamId = String(input.team_id ?? "");

    if (!UtilityInsightsService.UUID.test(teamId)) {
      throw Error("team not found");
    }

    await this.assertTeam(user, teamId);

    const mapName = UtilityInsightsService.text(input.map_name);
    const limit = UtilityInsightsService.clamp(
      input.limit,
      UtilityInsightsService.DEFAULT_REPORT_LIMIT,
      UtilityInsightsService.MAX_REPORT_LIMIT,
    );
    const radius = await this.lineups.successRadius();
    ...
    >(
      `WITH roster AS (
       ...
       SELECT nearest.id::text AS utility_lineup_id,
              count(*)::int AS thrown,
         ...
         CROSS JOIN LATERAL (
           SELECT l.id,
            ...
            WHERE l.lineup_bucket = t.lineup_bucket
              AND l.archived_at IS NULL
              AND public.can_view_utility_lineup(l, $3::json)
        ...
        LIMIT $5::int`,
      [teamId, mapName, UtilityInsightsService.session(user), radius, limit],
    );
    ...
    if (entries.length > 0) {
      return {
        ...
        entries: entries.map((entry) => ({
          utility_lineup_id: entry.utility_lineup_id,
  ...
  private async assertTeam(user: User, teamId: string): Promise<void> {
    ...
    >(
      `SELECT EXISTS (SELECT 1 FROM public.teams t WHERE t.id = $1::uuid) AS present,
              public.is_utility_team_member($1::uuid, $2::bigint) AS member`,
