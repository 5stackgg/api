...
export type SolveUtilityLineupInput = {
  session_id: string;
  target_x: number;
  target_y: number;
  target_z: number;
...
// posting it to POST /utility/ingest itself, so nothing here ever waits for a
...
export class UtilitySolverService {
  ...
  private static readonly STATUSES: ReadonlyArray<string> = [
    UtilitySolverStatus.Ready,
    UtilitySolverStatus.NoSample,
    UtilitySolverStatus.LaunchModelMismatch,
    UtilitySolverStatus.SeedReplayMismatch,
    UtilitySolverStatus.SeedReplayTimedOut,
    UtilitySolverStatus.Unsupported,
    UtilitySolverStatus.Unknown,
  ];
  ...
  private async claimSolveBudget(
    steamId: string,
  ): Promise<UtilitySolveOutput | null> {
    const perHour = Number(
      await this.setting(SystemSettingName.UtilitySolvesPerHour),
    );
    const cap =
      Number.isFinite(perHour) && perHour > 0
        ? perHour
        : UtilitySolverService.SOLVES_PER_HOUR;
    ...
    const countKey = `utility-solve-count:${steamId}:${hour}`;
    ...
    if (used >= cap) {
      ...
      return {
        accepted: false,
        status: UtilitySolverStatus.Busy,
    ...
    if (
      !(await this.cache.acquireLock(
        `utility-solve:${steamId}`,
        UtilitySolverService.SOLVE_LOCK_SECONDS,
      ))
    ) {
      return {
        accepted: false,
  ...
  public async calibration(
    user: User,
    sessionId: string,
  ): Promise<UtilityCalibrationOutput> {
  ...
  public async solve(
    ...
    const target = UtilitySolverService.point(
      input.target_x,
      input.target_y,
      input.target_z,
      "target",
    ...
    const args = [
      `target=${UtilitySolverService.vector(target)}`,
      ...(from ? [`from=${UtilitySolverService.vector(from)}`] : []),
      ...(input.utility_type
        ? [`utility=${UtilitySolverService.token(input.utility_type)}`]
        : []),
      `steam=${user.steam_id}`,
      ...(input.name ? [`name=${UtilitySolverService.token(input.name)}`] : []),
      ...(input.tolerance
        ? [
            `tolerance=${UtilitySolverService.number(input.tolerance, "tolerance")}`,
          ]
        : []),
    ];
    ...
    const reply = await this.send(
      ...
      `utility_solver_solve ${args.join(" ")}`,
    );

    if (reply === null) {
      return {
        accepted: false,
        status: UtilitySolverStatus.Unreachable,
        message: "unable to reach the practice server over rcon",
      };
    }

    this.logger.log(
      `[utility-solver] ${session.server.session_id} solving for ${user.steam_id}: ${UtilitySolverService.clean(reply) || "no reply"}`,
    );

    // The plugin answers the RCON as soon as the search is queued and posts the
    // winning lineup to /utility/ingest itself minutes later, so "accepted" is
    // the only thing that can honestly be said here.
    return {
      accepted: true,
      status: UtilitySolverStatus.Solving,
      message: UtilitySolverService.clean(reply) || null,
    };
  }
  ...
  private async calibrate(
    ...
    if (
      ...
      UtilitySolverService.UNSUPPORTED_RUNTIME
    ) {
      return {
        status: UtilitySolverStatus.Unsupported,
        ready: false,
        detail:
    ...
    const reply = await this.send(server.server_id, "utility_solver_calibrate");
    ...
    return UtilitySolverService.readCalibration(reply);
  }
  ...
  public static readCalibration(reply: string): UtilityCalibrationOutput {
    const text = UtilitySolverService.clean(reply);
    const spoken = text.match(UtilitySolverService.VERDICT)?.at(1) ?? "";

    const status = UtilitySolverService.STATUSES.find(
    ...
    if (status) {
      return {
        status,
        ready: status === UtilitySolverStatus.Ready,
        detail: text || null,
      };
    }
    ...
    return {
      status: UtilitySolverStatus.Unknown,
      ready: false,
  ...
  > {
    if (!UtilitySolverService.UUID.test(String(sessionId ?? ""))) {
    ...
    if (row.host_steam_id !== user.steam_id) {
      return {
        refusal: {
          status: UtilitySolverStatus.NotHost,
          message: "only the session host can drive the solver",
        },
      };
    }

    if (row.status !== UtilitySolverService.SOLVABLE_STATUS) {
      return {
        refusal: {
          status: UtilitySolverStatus.NotLive,
          message: `this practice session is ${row.status}, not ${UtilitySolverService.SOLVABLE_STATUS}`,
        },
      };
    }
    ...
    if (!row.server_id) {
      return {
        refusal: {
          status: UtilitySolverStatus.NoServer,
  ...
  private async send(
    ...
    try {
      ...
      this.logger.warn(
        `[utility-solver] ${serverId} rcon failed: ${(error as Error)?.message}`,
      );
      return null;
    }
  }
