jest.mock("@kubernetes/client-node", () => ({
  KubeConfig: jest.fn(),
  BatchV1Api: jest.fn(),
  CoreV1Api: jest.fn(),
  Exec: jest.fn(),
}));

import DamageEvent from "./DamageEvent";

function createEvent(data: Record<string, any>) {
  const hasura = { mutation: jest.fn().mockResolvedValue({}) };
  const logger = { log: jest.fn(), error: jest.fn(), warn: jest.fn(), debug: jest.fn() };
  const matchAssistant = {};
  const chat = {};
  const event = new DamageEvent(logger as any, hasura as any, matchAssistant as any, chat as any);
  event.setData("match-1", data as any);
  return { event, hasura };
}

describe("DamageEvent", () => {
  it("inserts damage with attacker data when attacker_steam_id is present", async () => {
    const { event, hasura } = createEvent({
      time: "2026-01-01T00:00:00Z",
      round: 3,
      attacker_steam_id: BigInt("76561198000000001"),
      attacker_team: "CT",
      attacker_location: "MidDoors",
      attacker_location_coordinates: "100.0 200.0 0.0",
      weapon: "ak47",
      damage: 27,
      damage_armor: 3,
      health: 73,
      armor: 97,
      hitgroup: "chest",
      attacked_steam_id: BigInt("76561198000000002"),
      attacked_team: "TERRORIST",
      attacked_location: "BombsiteA",
      attacked_location_coordinates: "300.0 400.0 0.0",
      match_map_id: "map-1",
    });

    await event.process();

    expect(hasura.mutation).toHaveBeenCalledWith({
      insert_player_damages_one: {
        __args: {
          object: {
            time: new Date("2026-01-01T00:00:00Z"),
            match_id: "match-1",
            round: 3,
            match_map_id: "map-1",
            with: "ak47",
            attacker_steam_id: BigInt("76561198000000001"),
            attacker_team: "CT",
            attacker_location: "MidDoors",
            attacker_location_coordinates: "100.0 200.0 0.0",
            attacked_steam_id: BigInt("76561198000000002"),
            attacked_team: "TERRORIST",
            attacked_location: "BombsiteA",
            damage: 27,
            damage_armor: 3,
            health: 73,
            armor: 97,
            hitgroup: "chest",
          },
        },
        __typename: true,
      },
    });
  });

  it("omits attacker fields when attacker_steam_id is falsy", async () => {
    const { event, hasura } = createEvent({
      time: "2026-01-01T00:00:00Z",
      round: 3,
      attacker_steam_id: 0,
      attacker_team: "",
      attacker_location: "",
      attacker_location_coordinates: "",
      weapon: "world",
      damage: 10,
      damage_armor: 0,
      health: 90,
      armor: 100,
      hitgroup: "generic",
      attacked_steam_id: BigInt("76561198000000002"),
      attacked_team: "CT",
      attacked_location: "BombsiteB",
      attacked_location_coordinates: "500.0 600.0 0.0",
      match_map_id: "map-1",
    });

    await event.process();

    const call = hasura.mutation.mock.calls[0][0];
    const obj = call.insert_player_damages_one.__args.object;
    expect(obj).not.toHaveProperty("attacker_steam_id");
    expect(obj).not.toHaveProperty("attacker_team");
    expect(obj).not.toHaveProperty("attacker_location");
    expect(obj).not.toHaveProperty("attacker_location_coordinates");
    expect(obj.damage).toBe(10);
    expect(obj.damage_armor).toBe(0);
    expect(obj.health).toBe(90);
    expect(obj.armor).toBe(100);
  });
});
