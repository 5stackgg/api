...
export class HasuraService {
  ...
  private async updateSettings() {
    ...
    // Anti-wallhack kill switch is on by default; seed the row so the admin
    // toggle reflects it. `do nothing` preserves an admin's explicit off.
    await this.postgresService.query(
      "insert into settings (name, value) values ('anti_wallhack_enabled', 'true') on conflict (name) do nothing",
    );
  }
