...
// coordinate bounds are the ones POST /nades/ingest enforces, shared rather
...
@Injectable()
export class NadeImportService {
  ...
  private static readonly NADE_TYPES: Record<string, string> = {
    smoke: "Smoke",
    smokegrenade: "Smoke",
    flash: "Flash",
    flashbang: "Flash",
  ...
  public async importLineups(
    ...
    for (const [index, raw] of entries.entries()) {
      ...
      try {
        lineup = NadeImportService.read(raw, maps);
      } catch (error) {
        output.failed += 1;
        NadeImportService.fail(
          output,
          index,
          NadeImportService.externalId(raw),
  ...
  public async purgeSource(
    ...
    const [known] = await this.postgres.query<Array<{ present: boolean }>>(
      `SELECT EXISTS (
                SELECT 1 FROM public.e_nade_sources s WHERE s.value = $1
              ) AS present`,
      [source],
    );
    ...
    await this.postgres.query(
      "DELETE FROM public.nade_lineups WHERE origin_source = $1",
      [source],
    );

    this.logger.log(
      `[nade-import] ${user.steam_id} purged ${lineups} '${source}' lineup(s)`,
    );
  ...
  private async existing(externalId: string): Promise<string | null> {
    const [row] = await this.postgres.query<Array<{ id: string }>>(
      ...
      [NadeImportService.ORIGIN_SOURCE, externalId],
    );
  ...
  private async write(
    ...
    const [row] = await this.postgres.query<Array<{ inserted: boolean }>>(
      ...
      [
        ...
        visibility,
        steamId,
        NadeImportService.ORIGIN_SOURCE,
        lineup.external_id,
      ],
    );
  ...
  private static read(raw: unknown, maps: Set<string>): Lineup {
    ...
    const externalId = NadeImportService.externalId(entry);
    ...
    const mapName = String(
      NadeImportService.pick(entry, ["map_name", "map"]) ?? "",
    ).trim();
    ...
    const nadeType = NadeImportService.classify(
      NadeImportService.pick(entry, [
        "nade_type",
        "type",
        "utility",
        "utility_type",
      ...
      NadeImportService.NADE_TYPES,
      "grenade type",
    );

    const origin = NadeImportService.vector(
      entry,
      ["origin", "position", "from", "start", "throw_position"],
      "origin",
    );
    const land = NadeImportService.vector(
      entry,
    ...
    const angles = NadeImportService.angles(entry);
    const flightTimeMs = NadeImportService.optionalNumber(
      NadeImportService.pick(entry, ["flight_time_ms", "flight_time"]),
      "flight_time_ms",
    );
    ...
    return {
      ...
      map_name: mapName,
      nade_type: nadeType,
      side: NadeImportService.classifyOr(
        NadeImportService.pick(entry, ["side", "team"]),
        NadeImportService.SIDES,
        "side",
        "TERRORIST",
      ),
      technique: NadeImportService.classifyOr(
        NadeImportService.pick(entry, [
          "technique",
          "movement",
          "throw",
        ...
        NadeImportService.TECHNIQUES,
        "technique",
        "Stationary",
      ),
      throw_strength: NadeImportService.classifyOrNull(
        NadeImportService.pick(entry, ["throw_strength", "strength"]),
        NadeImportService.STRENGTHS,
        "throw strength",
      ),
      jump_throw_bind:
        NadeImportService.pick(entry, [
      ...
      origin,
      land,
      eye_z: NadeImportService.optionalNumber(
        NadeImportService.pick(entry, ["eye_z", "eye_height"]),
        "eye_z",
      ),
      view_yaw: angles.yaw,
      ...
      name: `${nadeType} ${mapName} (${externalId})`.slice(0, 120),
    };
  }

  private static angles(entry: Entry): { yaw: number; pitch: number } {
    const grouped = NadeImportService.pick(entry, [
      "view",
      "angles",
      "viewangles",
      "view_angles",
    ...
    let yaw = NadeImportService.pick(entry, ["view_yaw", "yaw"]);
    let pitch = NadeImportService.pick(entry, ["view_pitch", "pitch"]);
    ...
    if (Array.isArray(grouped)) {
      ...
      yaw = yaw ?? NadeImportService.pick(record, ["yaw", "y"]);
      pitch = pitch ?? NadeImportService.pick(record, ["pitch", "x"]);
    }
    ...
    const resolvedPitch = NadeLineupsService.finite(pitch, "view_pitch");
    ...
    return {
      yaw: NadeLineupsService.finite(yaw, "view_yaw"),
      pitch: resolvedPitch,
    };
  }
  ...
  private static vector(
    ...
    const grouped = NadeImportService.pick(entry, keys);

    if (Array.isArray(grouped)) {
      return NadeLineupsService.point(
        grouped.at(0),
        grouped.at(1),
        grouped.at(2),
        label,
    ...
    if (grouped && typeof grouped === "object") {
      const record = grouped as Entry;
      return NadeLineupsService.point(record.x, record.y, record.z, label);
    }

    const flat = NadeImportService.pick(entry, [`${label}_x`]);
    ...
    return NadeLineupsService.point(
      flat,
      NadeImportService.pick(entry, [`${label}_y`]),
      NadeImportService.pick(entry, [`${label}_z`]),
      label,
    );
  }
  ...
  private static classify(
    ...
    const resolved = NadeImportService.classifyOrNull(value, table, label);
  ...
  private static classifyOr(
    ...
    return NadeImportService.classifyOrNull(value, table, label) ?? fallback;
  }
  ...
  private static optionalNumber(value: unknown, label: string): number | null {
    ...
    return NadeLineupsService.finite(value, label);
  }
  ...
  private static externalId(raw: unknown): string | null {
    ...
    const value = NadeImportService.pick(raw as Entry, [
      "external_id",
      "id",
      "key",
    ]);
  ...
  private static envelope(payload: unknown): {
    ...
    if (Array.isArray(payload)) {
      return { entries: NadeImportService.bounded(payload), visibility: null };
    }
    ...
    const entries = NadeImportService.pick(record, [
      "lineups",
      "entries",
      "nades",
      "data",
    ...
    return {
      entries: NadeImportService.bounded(entries),
      visibility: record.visibility,
    };
  }

  private static bounded(entries: Array<unknown>): Array<unknown> {
    if (entries.length > NadeImportService.MAX_ENTRIES) {
      throw Error(
        `payload holds more than ${NadeImportService.MAX_ENTRIES} lineups; split it`,
      );
    }
  ...
  private static fail(
    output: NadeImportOutput,
    index: number,
    externalId: string | null,
    reason: string,
  ): void {
    if (output.errors.length >= NadeImportService.MAX_ERRORS) {
      return;
    }
