...
import {
  ...
  NadeTrajectoryPoint,
} from "./nade-artifacts.service";

export type NadeIngestPayload = {
  match_id?: string;
...
export type NadePracticeResultPayload = {
  session_id?: string;
...
export type NadePracticeResult = {
  success: boolean;
  distance: number;
  radius: number;
...
@Injectable()
export class NadeLineupsService {
  ...
  // A player throwing flat out manages a nade every two or three seconds, and
  ...
  public async isLibraryEnabled(): Promise<boolean> {
    return (
      (await this.setting(SystemSettingName.NadeLibraryEnabled)) !== "false"
    );
  }
  ...
  public async serverContext(
    serverId: string,
  ): Promise<NadeServerContext | null> {
  ...
  public async ingest(
    ...
    const origin = NadeLineupsService.point(
      payload.origin_x,
      payload.origin_y,
      payload.origin_z,
      "origin",
    );
    const land = NadeLineupsService.point(
      payload.land_x,
      payload.land_y,
      payload.land_z,
      "land",
    ...
    if (
      NadeLineupsService.distance(origin, land) > NadeLineupsService.MAX_TRAVEL
    ) {
      throw Error(
    ...
    const yaw = NadeLineupsService.finite(payload.view_yaw, "view_yaw");
    const pitch = NadeLineupsService.finite(payload.view_pitch, "view_pitch");
    ...
    const flightTimeMs =
      payload.flight_time_ms === null || payload.flight_time_ms === undefined
        ? null
        : NadeLineupsService.finite(payload.flight_time_ms, "flight_time_ms");

    if (
      flightTimeMs !== null &&
      (flightTimeMs < NadeLineupsService.MIN_FLIGHT_MS ||
        flightTimeMs > NadeLineupsService.MAX_FLIGHT_MS)
    ) {
    ...
    const eyeZ =
      payload.eye_z === null || payload.eye_z === undefined
        ? null
        : NadeLineupsService.finite(payload.eye_z, "eye_z");
    ...
    const [inserted] = await this.postgres.query<Array<{ id: string }>>(
      `INSERT INTO public.nade_lineups
         (map_name, nade_type, side, technique, throw_strength, jump_throw_bind,
      ...
      [
        ...
        repair
          ? repair.lineup_name
          : NadeLineupsService.sanitizeName(payload.name, context.mapName),
        NadeLineupsService.sanitizeText(payload.description, 1000),
        author,
        context.matchId,
        ...
        seed ? "exact" : NadeLineupsService.DEFAULT_PLUGIN_CONFIDENCE,
        JSON.stringify(NadeLineupsService.preview(path)),
        repair?.nade_lineup_id ?? null,
      ],
    );
    ...
    if (path.length > 0) {
      trajectoryFile = await this.artifacts.uploadTrajectory({
        ...
        nadeType: String(payload.nade_type),
        ...
        tickRate: Number.isFinite(payload.tick_rate)
          ...
          : NadeLineupsService.DEFAULT_TICK_RATE,
        path,
      });
    }
      ...
      `SELECT l.id::text AS id, l.name, l.map_name, l.nade_type, l.side,
        ...
        WHERE l.map_name = $1
          ...
          AND (
            ...
            OR (l.visibility = 'Team' AND public.is_nade_team_member(l.team_id, $2::bigint))
            OR EXISTS (
              SELECT 1
                FROM public.nade_collection_items ci
                INNER JOIN public.nade_collections c ON c.id = ci.collection_id
               WHERE ci.nade_lineup_id = l.id
                 AND (
                   c.owner_steam_id = $2::bigint
                   OR (c.visibility = 'Team' AND public.is_nade_team_member(c.team_id, $2::bigint))
                 )
            )
            OR EXISTS (
              SELECT 1 FROM public.nade_lineup_favorites f
               WHERE f.nade_lineup_id = l.id AND f.steam_id = $2::bigint
            )
          )
  ...
  public async recordPracticeResult(
    context: NadeServerContext,
    payload: NadePracticeResultPayload,
  ): Promise<NadePracticeResult> {
    ...
    const lineupId = String(payload.nade_lineup_id ?? "");

    if (!NadeLineupsService.UUID.test(lineupId)) {
      throw Error("nade_lineup_id is not a lineup");
    }
    ...
    const distance = NadeLineupsService.distance(land, {
    ...
    const offset = NadeLineupsService.decomposeMiss(lineup, land);

    if (payload.success === true && !success) {
      this.logger.warn(
        `[nade-practice ${context.serverId}] plugin scored a hit on ${lineupId} that landed ${distance.toFixed(1)} units away`,
      );
    }
    ...
    >(
      `INSERT INTO public.nade_lineup_progress
       ...
       ON CONFLICT (nade_lineup_id, steam_id) DO UPDATE
          SET attempts = nade_lineup_progress.attempts + 1,
              successes = nade_lineup_progress.successes + $3::int,
              current_streak = CASE WHEN $3::int = 1
                                    THEN nade_lineup_progress.current_streak + 1
                                    ELSE 0 END,
              best_streak = GREATEST(
                  nade_lineup_progress.best_streak,
                  CASE WHEN $3::int = 1
                       THEN nade_lineup_progress.current_streak + 1
                       ELSE 0 END),
              ...
              mastered_at = COALESCE(
                  nade_lineup_progress.mastered_at,
                  CASE WHEN $3::int = 1
                            AND nade_lineup_progress.current_streak + 1 >= $4::int
                       THEN now() END),
              miss_samples = nade_lineup_progress.miss_samples + 1,
              miss_along_sum = nade_lineup_progress.miss_along_sum + $5::float8,
              miss_lateral_sum = nade_lineup_progress.miss_lateral_sum + $6::float8,
              miss_vertical_sum = nade_lineup_progress.miss_vertical_sum + $7::float8
       RETURNING attempts, successes, current_streak, best_streak, mastered_at`,
      [
        lineupId,
        steamId,
        success ? 1 : 0,
        NadeLineupsService.MASTERY_STREAK,
        offset.along,
        offset.lateral,
        offset.vertical,
      ],
    ...
    return {
      ...
      radius,
      along: NadeLineupsService.tenth(offset.along),
      lateral: NadeLineupsService.tenth(offset.lateral),
      vertical: NadeLineupsService.tenth(offset.vertical),
  ...
  public async successRadius(): Promise<number> {
    const configured = Number(
      await this.setting(SystemSettingName.NadeSuccessRadius),
    );

    if (!Number.isFinite(configured) || configured <= 0) {
      return NadeLineupsService.DEFAULT_SUCCESS_RADIUS;
    }
  ...
  public async deleteLineup(lineupId: string, steamId: string): Promise<void> {
    ...
    >(
      `SELECT id::text AS id, author_steam_id::text AS author_steam_id
         FROM public.nade_lineups WHERE id = $1::uuid`,
      [lineupId],
    );
    ...
    await this.postgres.query(
      "DELETE FROM public.nade_lineups WHERE id = $1::uuid",
      [lineupId],
    );
  }
  ...
  public async saveFromPractice(options: {
    ...
    const [row] = await this.postgres.query<Array<{ id: string }>>(
      `SELECT id::text AS id FROM public.nade_lineups
        WHERE id = $1::uuid
    ...
    await this.postgres.query(
      ...
      [
        options.lineupId,
        NadeLineupsService.sanitizeName(options.name, "Lineup"),
        NadeLineupsService.sanitizeText(options.description, 1000),
    ...
    if (options.collectionId) {
      await this.postgres.query(
        `INSERT INTO public.nade_collection_items (collection_id, nade_lineup_id)
         SELECT $1::uuid, $2::uuid
          WHERE EXISTS (
            SELECT 1 FROM public.nade_collections c
  ...
  // external id. The fork's own provenance is forked_from_nade_lineup_id plus
  ...
  // measure them -- tbiu_nade_lineups treats 'fork' as an origin that may hold
  ...
  public async fork(
    ...
    const sourceId = String(input.nade_lineup_id ?? "");

    if (!NadeLineupsService.UUID.test(sourceId)) {
      throw Error("lineup not found");
    }
    ...
    const [forked] = await this.postgres.query<Array<{ id: string }>>(
      `INSERT INTO public.nade_lineups
         (map_name, workshop_map_id, nade_type, side, technique, throw_strength,
          ...
          origin_source, forked_from_nade_lineup_id, confidence,
          trajectory_preview)
       SELECT l.map_name, l.workshop_map_id, l.nade_type, l.side, l.technique,
      ...
      [
        sourceId,
        NadeLineupsService.sanitizeText(input.name, 120),
        user.steam_id,
        JSON.stringify({
  ...
  public async assertDailyLineupLimit(steamId: string): Promise<void> {
    const limit = Number(
      (await this.setting(SystemSettingName.NadeLineupDailyLimit)) ?? "200",
    );
    ...
    const [row] = await this.postgres.query<Array<{ count: string }>>(
      `SELECT COUNT(*) AS count FROM public.nade_lineups
  ...
  private async assertResultRateLimit(
    ...
    const key = `nade-result:${serverId}:${steamId}:${Math.floor(
      Date.now() / 60000,
    )}`;
    ...
    if (count > NadeLineupsService.RESULTS_PER_MINUTE) {
  ...
  private async assertRateLimits(
    ...
    >(
      `SELECT
        ...
        FROM public.nade_lineups
       WHERE created_at > now() - interval '1 minute'
         AND (source_match_id = $1::uuid OR author_steam_id = $2::bigint)`,
      [context.matchId, author],
    );

    if (Number(row.per_server) >= NadeLineupsService.PER_SERVER_PER_MINUTE) {
      throw Error("this server is ingesting lineups too quickly");
    }

    if (Number(row.per_author) >= NadeLineupsService.PER_AUTHOR_PER_MINUTE) {
  ...
  private async claimableRepair(
    ...
    const repairId = NadeLineupsService.repairIdFromName(name);
    ...
    const [row] = await this.postgres.query<
      Array<{ id: string; nade_lineup_id: string; lineup_name: string }>
    >(
      `SELECT r.id::text AS id,
              r.nade_lineup_id::text AS nade_lineup_id,
              l.name AS lineup_name
         FROM public.nade_lineup_repairs r
         INNER JOIN public.nade_lineups l ON l.id = r.nade_lineup_id
        WHERE r.id = $1::uuid
  ...
  private async completeRepair(
    ...
    await this.postgres.query(
      `UPDATE public.nade_lineup_repairs
          SET status = 'Repaired',
              repaired_nade_lineup_id = $2::uuid,
              repaired_at = now()
  ...
  public static repairName(repairId: string): string {
    return `${NadeLineupsService.REPAIR_NAME_PREFIX}${repairId}`;
  }
  ...
  private static repairIdFromName(name: unknown): string | null {
    return (
      String(name ?? "")
        .trim()
        .match(NadeLineupsService.REPAIR_NAME)
        ?.at(1) ?? null
    );
  }
  ...
  private trajectory(
    path: NadeIngestPayload["path"],
  ): Array<NadeTrajectoryPoint> {
    if (!path) {
      return [];
    }
    ...
    if (path.length > NadeLineupsService.MAX_PATH_POINTS) {
      throw Error("path has too many points");
    }

    return path.map((point, index) => {
      const resolved = NadeLineupsService.point(
        point?.x,
        point?.y,
        point?.z,
        `path[${index}]`,
  ...
  private seed(payload: NadeIngestPayload): {
    ...
    const position = NadeLineupsService.point(
      payload.initial_pos_x,
      payload.initial_pos_y,
      payload.initial_pos_z,
      "initial_pos",
    ...
    const velocity = {
      x: NadeLineupsService.finite(payload.initial_vel_x, "initial_vel_x"),
      y: NadeLineupsService.finite(payload.initial_vel_y, "initial_vel_y"),
      z: NadeLineupsService.finite(payload.initial_vel_z, "initial_vel_z"),
    };
    ...
    if (speed > NadeLineupsService.MAX_VELOCITY) {
  ...
  public static point(
    ...
    return {
      x: NadeLineupsService.coord(x, `${label}.x`),
      y: NadeLineupsService.coord(y, `${label}.y`),
      z: NadeLineupsService.coord(z, `${label}.z`),
    };
  }

  public static coord(value: unknown, label: string): number {
    const resolved = NadeLineupsService.finite(value, label);

    if (Math.abs(resolved) > NadeLineupsService.MAX_COORD) {
  ...
  public static decomposeMiss(
    ...
    const yaw = (NadeLineupsService.throwAxis(lineup) * Math.PI) / 180;
  ...
  private static throwAxis(lineup: {
    ...
    if (travel < NadeLineupsService.MIN_AXIS_TRAVEL) {
    ...
    return disagreement > NadeLineupsService.MAX_AXIS_DISAGREEMENT_DEGREES
      ? bearing
      : yaw;
  }
  ...
  public static sanitizeName(
    ...
    const name = NadeLineupsService.sanitizeText(value, 120);
