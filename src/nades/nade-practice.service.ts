...
export type StartNadePracticeInput = {
  map_name: string;
  region?: string | null;
  collection_id?: string | null;
  team_id?: string | null;
...
@Injectable()
export class NadePracticeService {
  ...
  public static startLockKey(steamId: string): string {
    return `nade-practice:start:${steamId}`;
  }
  ...
  public async start(
    user: User,
    input: StartNadePracticeInput,
  ): Promise<NadePracticeSession> {
    if (!(await this.isEnabled())) {
      throw Error("nade practice is not enabled");
    }

    await this.assertRole(user);

    const lockKey = NadePracticeService.startLockKey(user.steam_id);

    if (
      !(await this.cache.acquireLock(
        lockKey,
        NadePracticeService.START_LOCK_SECONDS,
      ))
    ) {
    ...
    try {
      ...
      try {
        ...
        await this.postgres.query(
          "UPDATE public.nade_practice_sessions SET match_id = $2::uuid WHERE id = $1::uuid",
          [session.id, matchId],
        );
  ...
  public async join(
    ...
    if (!NadePracticeService.LIVE_STATUSES.includes(session.status)) {
    ...
    await this.matchAssistant.sendNadePracticeRefresh(session.match_id);
  ...
  public async invite(
    ...
    const invited = await this.postgres.query<Array<{ steam_id: string }>>(
      `INSERT INTO public.nade_practice_invites
         (nade_practice_session_id, steam_id, invited_by_steam_id)
  ...
  private async notifyInvited(
    ...
    try {
      await this.notifications.notifyPlayers(
        "NadePracticeInvite" as e_notification_types_enum,
        {
          title: "Nade Practice Invite",
          message:
            `<b>${host}</b> invited you to practise nades on <b>${map}</b>. ` +
      ...
      this.logger.warn(
        `[nade-practice] unable to notify invitees of ${session.id}: ${(error as Error)?.message}`,
      );
    }
  }
  ...
  public async loadPlaybook(
    ...
    await this.postgres.query(
      "UPDATE public.nade_practice_sessions SET playbook_id = $2::uuid WHERE id = $1::uuid",
  ...
  public async sessionForMatch(
    matchId: string,
  ): Promise<NadePracticeSession | null> {
    const [row] = await this.postgres.query<Array<NadePracticeSession>>(
      `${NadePracticeService.SELECT} WHERE s.match_id = $1::uuid`,
      [matchId],
    );
    return row ?? null;
  }
  ...
  private async assertInviteLookupRateLimit(steamId: string): Promise<void> {
    const key = `nade-invite-lookup:${steamId}:${Math.floor(
      Date.now() / 60000,
    )}`;
    ...
    if (count > NadePracticeService.INVITE_LOOKUPS_PER_MINUTE) {
  ...
  public async session(sessionId: string): Promise<NadePracticeSession | null> {
    const [row] = await this.postgres.query<Array<NadePracticeSession>>(
      `${NadePracticeService.SELECT} WHERE s.id = $1::uuid`,
      [sessionId],
    );
    return row ?? null;
  }

  // The plugin polls, so this runs on every GET /nades/session. Only the row
  ...
  public async markReady(matchId: string): Promise<void> {
    ...
    >(
      `UPDATE public.nade_practice_sessions
  ...
  private async notifyReady(
    ...
    try {
      await this.notifications.notifyPlayers(
        "NadePracticeReady" as e_notification_types_enum,
        {
          title: "Practice Server Ready",
          message:
            `Your nade practice server on <b>${map}</b> is up. ` +
      ...
      this.logger.warn(
        `[nade-practice] unable to announce ${session.id} as ready: ${(error as Error)?.message}`,
      );
    }
  }
  ...
  // the join flow is built around, and it is what joinNadePractice takes on
  // this path.
  private inviteUrl(session: NadePracticeSession): string {
  ...
  private mapUrl(mapName: string): string {
    return `${this.appConfig.webDomain}/nades/${encodeURIComponent(mapName)}`;
  }
  ...
  public async touch(sessionId: string): Promise<void> {
    await this.postgres.query(
      "UPDATE public.nade_practice_sessions SET last_occupied_at = now() WHERE id = $1::uuid",
      [sessionId],
    );
  }
  ...
  public async sessionForServer(serverId: string): Promise<{
    ...
    playbook: NadePlaybookPayload | null;
  } | null> {
  ...
  public async liveSessionForServer(serverId: string): Promise<{
    ...
    >(
      `SELECT s.id::text AS session_id,
         ...
         INNER JOIN public.nade_practice_sessions s ON s.match_id = m.id
        WHERE srv.id = $1::uuid
  ...
  public async reapIdle(): Promise<number> {
    const idleMinutes = Number(
      (await this.setting(SystemSettingName.NadePracticeIdleMinutes)) ?? "10",
    );
    const idle =
      Number.isFinite(idleMinutes) && idleMinutes > 0 ? idleMinutes : 10;

    const sessions = await this.postgres.query<Array<NadePracticeSession>>(
      `${NadePracticeService.SELECT} WHERE s.status IN ('Starting', 'Ready')`,
    );
    ...
    for (const session of sessions) {
      try {
        ...
        if (
          ...
          NadePracticeService.olderThanMinutes(
            session.created_at,
            NadePracticeService.BOOT_GRACE_MINUTES,
          )
        ) {
        ...
        // GET /nades/session is the definitive Ready signal -- the plugin is up
        ...
        const [row] = await this.postgres.query<Array<{ stale: boolean }>>(
          `SELECT empty_since IS NOT NULL
             ...
             FROM public.nade_practice_sessions WHERE id = $1::uuid`,
        ...
        this.logger.warn(
          `[nade-practice ${session.id}] reaper failed: ${(error as Error)?.message}`,
        );
      }
    }

    if (reaped > 0) {
      this.logger.log(`reaped ${reaped} idle nade practice session(s)`);
    }
  ...
  private async end(
    session: NadePracticeSession | null,
    status: "Ended" | "Failed",
  ): Promise<void> {
    if (!session) {
      return;
    ...
    await this.postgres.query(
      "UPDATE public.nade_practice_sessions SET status = $2 WHERE id = $1::uuid",
      [session.id, status],
    );
  ...
  private async insertSession(
    ...
    // generate_nade_invite_code() is 50 bits, so a collision is vanishingly
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        const [row] = await this.postgres.query<Array<{ id: string }>>(
          `INSERT INTO public.nade_practice_sessions
        ...
        if (message.includes("nade_practice_sessions_one_live_per_host_idx")) {
          throw Error("you already have a practice session running");
        }

        if (
          attempt < 3 &&
          message.includes("nade_practice_sessions_invite_code_idx")
        ) {
          continue;
        }
    ...
    const { insert_match_options_one: matchOptions } =
      await this.hasura.mutation({
        insert_match_options_one: {
          __args: {
            object: {
              type: NadePracticeService.MATCH_TYPE,
              ...
              number_of_substitutes: NadePracticeService.SUBSTITUTES,
              knife_round: false,
              overtime: false,
              coaches: false,
              mr: 12,
      ...
      `SELECT (
         EXISTS (
           SELECT 1 FROM public.nade_practice_invites i
            WHERE i.nade_practice_session_id = $1::uuid AND i.steam_id = $2::bigint
         )
         OR public.is_nade_team_member($3::uuid, $2::bigint)
         OR EXISTS (
  ...
  private async assertServerHeadroom(region: string): Promise<void> {
    const reserved = Number(
      (await this.setting(SystemSettingName.NadePracticeReservedServers)) ??
        "2",
    );
  ...
  private async assertDailyLimit(steamId: string): Promise<void> {
    const limit = Number(
      (await this.setting(SystemSettingName.NadePracticeDailyLimit)) ?? "10",
    );
    ...
    const [row] = await this.postgres.query<Array<{ count: string }>>(
      `SELECT COUNT(*) AS count FROM public.nade_practice_sessions
  ...
  private async resolveMap(mapName: string): Promise<string> {
    const [row] = await this.postgres.query<Array<{ name: string }>>(
      `SELECT name FROM public.maps
        ...
        LIMIT 1`,
      [mapName, NadePracticeService.MATCH_TYPE],
    );
