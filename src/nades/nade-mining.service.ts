...
export type NadeUtilityTypeReport = {
  nade_type: string;
  throws: number;
...
export type NadeUtilityReport = {
  ...
  landed: number;
  radius: number;
  by_type: Array<NadeUtilityTypeReport>;
};
...
@Injectable()
export class NadeMiningService {
  ...
  private static readonly MAX_PLAUSIBLE_GRAVITY =
    2 * NadeMiningService.SOURCE_GRAVITY;
  ...
  public async saveFromDemo(
    ...
    const mined = NadeMiningService.mine(blob, grenadeId, mapName);
    ...
    const [inserted] = await this.postgres.query<Array<{ id: string }>>(
      ...
      [
        ...
        mined.land.z,
        mined.flightTimeMs,
        NadeLineupsService.sanitizeName(request.name, mined.mapName),
        NadeLineupsService.sanitizeText(request.description, 1000),
        (request.tags ?? [])
          .slice(0, 16)
        ...
        mined.grenadeId,
        JSON.stringify(NadeLineupsService.preview(mined.path)),
      ],
    );
    ...
    this.logger.log(
      `[nade-mining] ${request.user.steam_id} mined ${mined.nadeType} ${inserted.id} ` +
  ...
  // "You threw 12 nades; 4 matched known lineups; 3 landed correctly."
  //
  // Computed on read, never stored. Every input is already a persisted, indexed
  // fact -- nade_demo_throws is the fact table, and both lineup_bucket columns
  ...
  public async utilityReport(
    ...
    >(
      `SELECT count(*) FILTER (WHERE d.playback_file IS NOT NULL) AS demos,
         ...
         LEFT JOIN public.nade_demo_mines mined
    ...
    const rows: Array<NadeUtilityTypeReport> = byType.map((row) => ({
      nade_type: row.nade_type,
    ...
    const total = (pick: (row: NadeUtilityTypeReport) => number) =>
  ...
  private async authorizeMatchMap(request: NadeMiningRequest): Promise<string> {
    const { matches } = await this.hasura.query(
      {
        matches: {
          __args: {
  ...
  public static mine(
    ...
    return NadeMiningService.mineThrow(
      NadeMiningService.index(blob, mapName),
      grenadeId,
    );
  }
  ...
  public static mineAll(
    ...
    const context = NadeMiningService.index(blob, mapName);
    const mined: Array<MinedLineup> = [];

    for (const grenadeId of context.thrown.keys()) {
      try {
        mined.push(NadeMiningService.mineThrow(context, grenadeId));
      } catch {
        continue;
      }
    }
  ...
  private static mineThrow(context: BlobIndex, grenadeId: number): MinedLineup {
    ...
    const nadeType = DemoParserService.utilityType(thrown.type);

    if (!nadeType) {
      throw Error(`unknown grenade type ${thrown.type}`);
    }

    const side = NadeMiningService.SIDES[String(thrown.thrower_team ?? "")];
    ...
    const release = NadeMiningService.releaseVelocity(points, tickRate);
    const samples = context.positions.get(thrown.thrower_steam_id ?? "") ?? [];
    const stance = NadeMiningService.stanceAt(samples, thrown.tick, tickRate);

    const carried = NadeMiningService.carriedVelocity(stance);
    ...
    const aim = NadeMiningService.anglesOf(impulse);
    ...
    const origin = stance?.position ?? {
      ...
      z: thrown.z - NadeMiningService.STANDING_EYE_HEIGHT,
    };
    ...
    return {
      ...
      nadeType,
      side,
      technique: NadeMiningService.techniqueOf(stance),
      throwStrength:
        release.trustedScale && carried
          ? NadeMiningService.strengthOf(NadeMiningService.magnitude(impulse))
          : null,
      throwSpeed:
        release.trustedScale && carried
          ? NadeMiningService.magnitude(impulse)
          : null,
      origin,
      eyeZ:
        origin.z +
        (stance?.ducked
          ? NadeMiningService.DUCKED_EYE_HEIGHT
          : NadeMiningService.STANDING_EYE_HEIGHT),
      viewYaw: aim.yaw,
      viewPitch: aim.pitch,
      viewYawDelta:
        recorded?.yaw === null || recorded?.yaw === undefined
          ? null
          : NadeMiningService.angleDelta(aim.yaw, Number(recorded.yaw)),
      viewPitchDelta:
        recorded?.pitch === null || recorded?.pitch === undefined
          ? null
          : NadeMiningService.angleDelta(aim.pitch, Number(recorded.pitch)),
      ...
      tickRate,
      path: NadeMiningService.pathOf(points, thrown.tick),
  ...
  private static releaseVelocity(
    ...
    tickRate: number,
  ): { velocity: NadeVector; gravity: number; trustedScale: boolean } {
    const sample = points.slice(0, NadeMiningService.FIT_POINTS);
    const base = sample[0].t;
    const ticks = sample.map((point) => point.t - base);

    const fitX = NadeMiningService.fitQuadratic(
      ticks,
      sample.map((point) => point.x),
    );
    const fitY = NadeMiningService.fitQuadratic(
      ticks,
      sample.map((point) => point.y),
    );
    const fitZ = NadeMiningService.fitQuadratic(
      ticks,
    ...
    return {
      ...
      gravity,
      trustedScale:
        gravity >= NadeMiningService.MIN_PLAUSIBLE_GRAVITY &&
        gravity <= NadeMiningService.MAX_PLAUSIBLE_GRAVITY,
    };
  }
  ...
  private static fitQuadratic(
    ...
    return NadeMiningService.solve3(
      [
        [s0, s1, s2],
        [s1, s2, s3],
        [s2, s3, s4],
  ...
  public static anglesOf(velocity: NadeVector): {
    yaw: number;
    pitch: number;
  } {
    const speed = NadeMiningService.magnitude(velocity);
  ...
  private static stanceAt(
    ...
    if (
      Math.abs(nearest.tick - tick) > NadeMiningService.MAX_SAMPLE_LAG_TICKS
    ) {
      return null;
    }
    ...
    let velocity: NadeVector | null = null;
    ...
    const window = sorted.filter(
      (sample) =>
        Math.abs(sample.tick - tick) <= NadeMiningService.MAX_SAMPLE_LAG_TICKS,
    );
  ...
  private static carriedVelocity(stance: Stance | null): NadeVector | null {
    if (!stance?.velocity || stance.velocityTicks === null) {
      return null;
    }

    if (stance.velocityTicks <= NadeMiningService.MAX_VELOCITY_PAIR_TICKS) {
      return stance.velocity;
    }

    return NadeMiningService.magnitude(stance.velocity) <
      NadeMiningService.STATIONARY_MAX_SPEED
      ? stance.velocity
      : null;
  }
  ...
  private static techniqueOf(
    ...
    const airborne =
      stance.rise >= NadeMiningService.JUMP_MIN_RISE ||
      vertical >= NadeMiningService.JUMP_MIN_VERTICAL_SPEED;

    if (airborne) {
      if (stance.ducked) {
        return "CrouchJump";
      }
      if (horizontal > NadeMiningService.WALK_MAX_SPEED) {
        return "RunJump";
      }
      if (horizontal > NadeMiningService.STATIONARY_MAX_SPEED) {
        return "WalkJump";
      }
      return "Jump";
    }
  ...
  public static strengthOf(speed: number): string | null {
    if (speed >= NadeMiningService.FULL_THROW_MIN_SPEED) {
      return "Full";
    }

    if (
      speed >= NadeMiningService.HALF_THROW_MIN_SPEED &&
      speed <= NadeMiningService.HALF_THROW_MAX_SPEED
    ) {
      return "Half";
    }

    if (speed <= NadeMiningService.DROP_THROW_MAX_SPEED) {
      return "Drop";
    }
  ...
  private static pathOf(
    ...
    const stride = Math.max(
      1,
      Math.ceil(points.length / NadeLineupsService.MAX_PATH_POINTS),
    );
    const path: Array<NadeTrajectoryPoint> = [];
  ...
  private static magnitude(vector: NadeVector): number {
  ...
  private static angleDelta(derived: number, recorded: number): number {
    ...
    return NadeMiningService.round(delta);
  }
