...
// that is kept, and no coordinate from it may ever be shown as "where your nade
// lands".
@Injectable()
export class NadeDriftService {
  ...
  public async startScan(
    ...
    const [scan] = await this.postgres.query<Array<{ id: string }>>(
      `INSERT INTO public.nade_drift_scans
         (map_name, from_revision, to_revision, lineups, requested_by_steam_id)
       VALUES ($1, $2, $3, $4::int, $5::bigint)
       RETURNING id::text AS id`,
      [
        mapName,
        NadeDriftService.revision(input.from_revision),
        NadeDriftService.revision(input.to_revision),
        Number(lineups),
        user.steam_id,
      ],
    );
  ...
  public async runScan(scanId: string): Promise<void> {
    ...
    >(
      `SELECT id::text AS id, map_name, from_revision, to_revision, status
         FROM public.nade_drift_scans
        WHERE id = $1::uuid`,
      [scanId],
    );

    if (!scan) {
      this.logger.warn(`[nade-drift] scan ${scanId} no longer exists`);
      return;
    }

    if (scan.status !== "Pending") {
      this.logger.warn(
        `[nade-drift] scan ${scanId} is ${scan.status}; refusing to run it twice`,
      );
      return;
    }

    await this.postgres.query(
      `UPDATE public.nade_drift_scans SET status = 'Running' WHERE id = $1::uuid`,
      [scan.id],
    );
    ...
    for (;;) {
      ...
      const answer = await this.demoParser.drift({
        ...
        lineups: batch.map((lineup) => NadeDriftService.seed(lineup)),
      });
      ...
      await this.postgres.query(
        `UPDATE public.nade_drift_scans SET scanned = $2::int WHERE id = $1::uuid`,
        [scan.id, scanned],
      );
    }
  ...
  private async batch(
    ...
    return await this.postgres.query<Array<SeededLineup>>(
      `SELECT id::text AS id, nade_type,
        ...
        ORDER BY id ASC
        LIMIT ${NadeDriftService.BATCH_SIZE}`,
      [mapName, after],
    );
  }
  ...
  private async record(
    ...
    for (const result of results) {
      ...
      await this.postgres.query(
        `INSERT INTO public.nade_drift_results
           (nade_drift_scan_id, nade_lineup_id, verdict, severity, reason,
            distance, distance_xy, distance_z)
         VALUES ($1::uuid, $2::uuid, $3, $4, $5, $6, $7, $8)
         ON CONFLICT (nade_drift_scan_id, nade_lineup_id) DO UPDATE
  ...
  private async finish(scanId: string): Promise<void> {
    ...
    >(
      `UPDATE public.nade_drift_scans s
          SET status = 'Finished',
         ...
         FROM (
           SELECT count(*) FILTER (WHERE verdict = 'unchanged')::int AS unchanged,
             ...
             FROM public.nade_drift_results
            WHERE nade_drift_scan_id = $1::uuid
         ) AS t
        WHERE s.id = $1::uuid
  ...
  private async fail(scanId: string, reason: string): Promise<void> {
    this.logger.warn(`[nade-drift] scan ${scanId} failed: ${reason}`);

    const [scan] = await this.postgres.query<Array<{ map_name: string }>>(
      `UPDATE public.nade_drift_scans
  ...
  private reviewLink(): string {
    return `<a href="${this.appConfig.webDomain}/nades/drift">Review the scan</a>.`;
  }
  ...
  private async announce(
    ...
    try {
      await this.notifications.send(
        "NadeDriftScanFinished" as e_notification_types_enum,
        {
          title,
          message,
      ...
      this.logger.warn(
        `[nade-drift] unable to announce scan ${scanId}: ${(error as Error)?.message}`,
      );
    }
  }
  ...
  private static seed(lineup: SeededLineup): ParsedDriftLineup {
    ...
    return {
      id: lineup.id,
      nade_type: DemoParserService.parserUtilityType(lineup.nade_type),
