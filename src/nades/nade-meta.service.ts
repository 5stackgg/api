...
import { MinedLineup, NadeMiningService } from "./nade-mining.service";
...
export const NADE_META_MINER_VERSION = 1;
...
export class NadeMetaService {
  ...
  public async mine(
    limit = NadeMetaService.DEMOS_PER_RUN,
    ...
    for (const demo of demos) {
      try {
        ...
        this.logger.warn(`[nade-meta] ${demo.id} failed to mine: ${reason}`);
    ...
    this.logger.log(
      `[nade-meta] mined ${total} throw(s) from ${demos.length} demo(s) across ${maps.size} map(s)`,
    );
  ...
  private async pendingDemos(limit: number): Promise<Array<PendingDemo>> {
    return await this.postgres.query<Array<PendingDemo>>(
      `SELECT d.id::text AS id,
         ...
         LEFT JOIN public.nade_demo_mines mined ON mined.match_map_demo_id = d.id
        WHERE d.playback_file IS NOT NULL
          AND (
            ...
            OR (
              ...
              AND mined.mined_at < now() - interval '${NadeMetaService.RETRY_AFTER}'
            )
          )
        ORDER BY d.created_at DESC
        LIMIT $2::int`,
      [NADE_META_MINER_VERSION, limit],
    );
  }
  ...
  public async mineDemo(demo: PendingDemo): Promise<number> {
    ...
    const mined = NadeMiningService.mineAll(
      blob,
  ...
  private async writeThrows(
    ...
    await this.postgres.query(
      `INSERT INTO public.nade_demo_throws
         (match_map_demo_id, grenade_id, match_id, match_map_id, map_name,
          nade_type, side, technique, throw_strength, thrower_steam_id,
       ...
       SELECT $1::uuid, v.grenade_id, $2::uuid, $3::uuid, $4,
              v.nade_type, v.side, v.technique, v.throw_strength, v.thrower_steam_id,
         ...
         FROM UNNEST(
              ...
              ) AS v(grenade_id, nade_type, side, technique, throw_strength,
       ...
       ON CONFLICT (match_map_demo_id, grenade_id) DO UPDATE
          SET nade_type = EXCLUDED.nade_type,
              side = EXCLUDED.side,
      ...
      [
        ...
        mined.map((throwOf) => throwOf.nadeType),
  ...
  private async recordMine(
    ...
    await this.postgres.query(
      `INSERT INTO public.nade_demo_mines
      ...
      [matchMapDemoId, NADE_META_MINER_VERSION, throws, failedReason],
    );
  }
  ...
  public async refreshClusters(
    ...
    const rows = await this.postgres.query<Array<{ lineup_bucket: string }>>(
      `INSERT INTO public.nade_meta_lineups
         (lineup_bucket, map_name, nade_type, side, technique, throw_strength,
       ...
       SELECT t.lineup_bucket,
              ...
              (SELECT COUNT(*)::int FROM public.nade_lineups l
        ...
        GROUP BY t.lineup_bucket, t.map_name, t.nade_type
    ...
    await this.postgres.query(
      `DELETE FROM public.nade_meta_lineups c
        WHERE ($1::text[] IS NULL OR c.map_name = ANY($1::text[]))
          AND NOT EXISTS (
            SELECT 1 FROM public.nade_demo_throws t
