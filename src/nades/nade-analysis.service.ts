...
export type NadeSightlinePairInput = {
  from_x: number;
  from_y: number;
  from_z: number;
  to_x: number;
...
export type NadeSightlineOutput = {
  threshold: number;
  results: Array<NadeSightlineResult>;
  degraded: boolean;
  message: string | null;
};
...
export type NadeOneWayOutput = {
  results: Array<NadeOneWayResult>;
  degraded: boolean;
  message: string | null;
};
...
export type NadePlaybookCoverageOutput = {
  results: Array<NadePlaybookCoverageResult>;
  degraded: boolean;
  message: string | null;
};
...
export type NadeBlockingOutput = {
  results: Array<NadeBlockingResult>;
  degraded: boolean;
  message: string | null;
};
...
@Injectable()
export class NadeAnalysisService {
  ...
  public async sightlines(
  ...
  ): Promise<NadeSightlineOutput> {
    const lineup = await this.analysableLineup(user, input.lineup_id);
    const pairs = NadeAnalysisService.pairs(input.pairs);
    const threshold = NadeAnalysisService.threshold(input.threshold);
    ...
    if (!answer.data) {
      return {
        threshold: threshold ?? NadeAnalysisService.DEFAULT_THRESHOLD,
        results: [],
        degraded: true,
    ...
    return {
      threshold:
        ...
        threshold ??
        NadeAnalysisService.DEFAULT_THRESHOLD,
  ...
  public async oneWay(
    user: User | null,
    input: { lineup_id: string; pairs: Array<NadeSightlinePairInput> },
  ): Promise<NadeOneWayOutput> {
    const lineup = await this.analysableLineup(user, input.lineup_id);
    const pairs = NadeAnalysisService.pairs(input.pairs);
  ...
  public async playbookCoverage(
  ...
  ): Promise<NadePlaybookCoverageOutput> {
    const pairs = NadeAnalysisService.pairs(input.pairs);
    ...
    if (steps.length === 0) {
      ...
      const uncovered = pairs.map(
        (_pair, index): NadePlaybookCoverageResult => ({
          index,
          covered: false,
          by_step: null,
          depth: 0,
    ...
    const evaluated = steps.slice(0, NadeAnalysisService.CANDIDATE_CAP);
    ...
    const threshold =
      answer.data.threshold ?? NadeAnalysisService.DEFAULT_THRESHOLD;
    ...
    return {
      ...
      degraded: truncated,
      message: truncated
        ? `only the first ${NadeAnalysisService.CANDIDATE_CAP} smokes in this playbook were evaluated`
        : null,
    };
  }
  ...
  public async findBlocking(
  ...
  ): Promise<NadeBlockingOutput> {
    const from = NadeAnalysisService.point(
      input.from_x,
      input.from_y,
      input.from_z,
    );
    const to = NadeAnalysisService.point(input.to_x, input.to_y, input.to_z);
    const requested = Math.trunc(Number(input.limit ?? 0));
    const limit = Math.min(
      requested > 0 ? requested : NadeAnalysisService.DEFAULT_BLOCKING_LIMIT,
      NadeAnalysisService.MAX_BLOCKING_LIMIT,
    );
    ...
    if (candidates.length === NadeAnalysisService.CANDIDATE_CAP) {
      this.logger.log(
        `[nade-blocking] ${mapName}: evaluating the ${NadeAnalysisService.CANDIDATE_CAP} smokes nearest the line; more were in reach`,
      );
    }
    ...
    if (pair.world_blocked) {
      const output: NadeBlockingOutput = {
        results: [],
        degraded: false,
      ...
      await this.cache.put(
        cacheKey,
        output,
        NadeAnalysisService.BLOCKING_TTL_SECONDS,
      );
      return output;
    }
  ...
  private async analysableLineup(
    ...
    if (!NadeAnalysisService.isUuid(lineupId)) {
    ...
    >(
      `SELECT public.can_view_nade_lineup(l, $2::json) AS visible,
              l.id::text AS id, l.map_name, l.nade_type,
              l.land_x, l.land_y, l.land_z, l.trajectory_file
         FROM public.nade_lineups l
        WHERE l.id = $1::uuid`,
      [lineupId, NadeAnalysisService.session(user)],
    );
  ...
  private async viewablePlaybook(
    ...
    if (!NadeAnalysisService.isUuid(playbookId)) {
    ...
    >(
      `SELECT p.id::text AS id, p.map_name,
        ...
        WHERE p.id = $1::uuid`,
      [playbookId, NadeAnalysisService.session(user)],
    );
  ...
  private async blockingCandidates(
    ...
    return await this.postgres.query<Array<AnalysableLineup>>(
      `WITH seg AS (
       ...
       SELECT l.id::text AS id, l.map_name, l.nade_type,
              l.land_x, l.land_y, l.land_z, l.trajectory_file
         FROM public.nade_lineups l
        CROSS JOIN seg
        CROSS JOIN LATERAL (
        ...
        WHERE l.map_name = $1
          ...
          AND public.can_view_nade_lineup(l, $8::json)
        ORDER BY gap.reach ASC, l.upvotes DESC, l.id ASC
        LIMIT ${NadeAnalysisService.CANDIDATE_CAP}`,
      [
        mapName,
        from.x,
        from.y,
        ...
        to.y,
        to.z,
        NadeAnalysisService.session(user),
        side ?? null,
        NadeAnalysisService.SMOKE_REACH,
      ],
    );
  }
  ...
  private async cloudFor(lineup: AnalysableLineup): Promise<ParsedCloudSpec> {
    const land = NadeAnalysisService.point(
      lineup.land_x,
      lineup.land_y,
      lineup.land_z,
    );
    const key = NadeAnalysisService.volumeCacheKey(lineup.map_name, land);
    ...
    await this.cache.put(key, volume, NadeAnalysisService.VOLUME_TTL_SECONDS);
  ...
  private blockingCacheKey(
    ...
    // can_view_nade_lineup, so two players asking the same question are not
    // asking the same question.
    const at = (point: ParsedGeometryPoint) =>
      `${Math.round(point.x)},${Math.round(point.y)},${Math.round(point.z)}`;

    return `nades:blocking:${user?.steam_id ?? "guest"}:${user?.role ?? "guest"}:${mapName}:${at(from)}:${at(to)}:${side ?? "any"}`;
  }
  ...
  private static volumeCacheKey(
    ...
    return `nades:volume:${mapName}:${Math.round(point.x)},${Math.round(point.y)},${Math.round(point.z)}`;
  }
  ...
  private static pairs(
    input: Array<NadeSightlinePairInput>,
  ): Array<ParsedSightlinePair> {
    if (!Array.isArray(input) || input.length === 0) {
      throw Error("at least one pair is required");
    }

    if (input.length > NadeAnalysisService.MAX_PAIRS) {
      throw Error(`at most ${NadeAnalysisService.MAX_PAIRS} pairs at a time`);
    }

    return input.map((pair) => ({
      from: NadeAnalysisService.point(pair.from_x, pair.from_y, pair.from_z),
      to: NadeAnalysisService.point(pair.to_x, pair.to_y, pair.to_z),
    }));
  }
