...
import { NadeImportService } from "./../src/nades/nade-import.service";
...
describe("nade lineup seeding (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("NadeImportTest");
  ...
  beforeEach(async () => {
    ...
    await postgres.query(
      "UPDATE settings SET value = 'true' WHERE name = 'public.nade_import_enabled'",
    );
  });
  ...
  function service(): NadeImportService {
    return new NadeImportService(new Logger("NadeImportTest"), postgres, {
  ...
  const rows = async () =>
    ...
    >(
      `SELECT id::text AS id, external_id, origin_source, confidence, verified_at,
              source_url, name, map_name, nade_type, side, technique,
              throw_strength, visibility, land_x,
              author_steam_id::text AS author_steam_id
         FROM nade_lineups ORDER BY external_id`,
    );
  ...
  describe("who may run it", () => {
    ...
    it("refuses while the setting is off", async () => {
      ...
      await postgres.query(
        "UPDATE settings SET value = 'false' WHERE name = 'public.nade_import_enabled'",
      );
    ...
    it("refuses when the setting has never been written", async () => {
      const op = await fx.player();
      await postgres.query(
        "DELETE FROM settings WHERE name = 'public.nade_import_enabled'",
      );
      ...
      await postgres.query(
        "INSERT INTO settings (name, value) VALUES ('public.nade_import_enabled', 'true')",
      );
    });
  });
  ...
  describe("what it writes", () => {
    it("maps an entry onto a low-confidence, unverified lineup", async () => {
      ...
      expect(row.nade_type).toBe("Smoke");
    ...
    it("reads the shapes an export can plausibly be in", async () => {
      ...
      >(
        "SELECT view_yaw, view_pitch FROM nade_lineups WHERE external_id = 'engine-order-angles'",
      );
  ...
  describe("per-entry validation", () => {
    it("reports each bad row and lands the good ones", async () => {
      ...
      const result = await service().importLineups(admin(op), {
        payload: [
          ...
          // The same bounds POST /nades/ingest enforces, shared rather than
    ...
    it("uses the same coordinate ceiling as the ingest door", async () => {
      ...
      const result = await service().importLineups(admin(op), {
        payload: [
          entry({
            id: "edge",
            land: { ...LAND, x: NadeLineupsService.MAX_COORD + 1 },
          }),
        ],
      });
    ...
    it("refuses a payload past the batch ceiling", async () => {
      ...
      const many = Array.from(
        { length: NadeImportService.MAX_ENTRIES + 1 },
  ...
  describe("undoing it", () => {
    ...
    it("clears the S3 artifacts of a source that has them", async () => {
      ...
      await seeded(op);
      await postgres.query(
        "UPDATE nade_lineups SET trajectory_file = 'key' WHERE origin_source = 'editor'",
      );
