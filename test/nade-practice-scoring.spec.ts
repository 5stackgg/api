...
describe("nade practice scoring (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("NadeScoringTest");
  ...
  describe("scoring a throw", () => {
    ...
    it("counts a throw inside the radius", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.radius).toBe(NadeLineupsService.DEFAULT_SUCCESS_RADIUS);
    });

    it("scores against the configured radius, not a constant", async () => {
      const player = await fx.player();
      const lineup = await insertLineup(player);
      await setting("public.nade_success_radius", "10");
      const service = makeService();
  ...
  describe("mastery", () => {
    async function throwAt(
      service: NadeLineupsService,
      ctx: NadeServerContext,
      lineup: string,
      player: string,
      offset: number,
    ) {
    ...
    it("masters a lineup on the consecutive-success bar and keeps it after a miss", async () => {
      ...
      for (let i = 1; i < NadeLineupsService.MASTERY_STREAK; i++) {
      ...
      expect(mastering.current_streak).toBe(NadeLineupsService.MASTERY_STREAK);
      ...
      expect(missed.best_streak).toBe(NadeLineupsService.MASTERY_STREAK);
      expect(missed.mastered_at).toEqual(mastering.mastered_at);
      expect(missed.attempts).toBe(NadeLineupsService.MASTERY_STREAK + 1);
      expect(missed.successes).toBe(NadeLineupsService.MASTERY_STREAK);
    });
    ...
    it("does not master a lineup hit the same number of times but not in a row", async () => {
      ...
      for (let i = 0; i < NadeLineupsService.MASTERY_STREAK; i++) {
        await throwAt(service, ctx, lineup, player, 0);
        await throwAt(service, ctx, lineup, player, 900);
      }

      const row = await progress(lineup, player);
      expect(row.successes).toBe(NadeLineupsService.MASTERY_STREAK);
  ...
  describe("rate limiting", () => {
    it("cuts off one player on one server without touching anybody else", async () => {
      ...
      for (let i = 0; i < NadeLineupsService.RESULTS_PER_MINUTE; i++) {
