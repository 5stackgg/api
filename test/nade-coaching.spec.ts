...
import { NadeInsightsService } from "./../src/nades/nade-insights.service";
import {
  NadeLineupsService,
  NadeServerContext,
...
describe("nade coaching (SQL-driven)", () => {
  let db: SqlTestDb;
  let postgres: PostgresService;
  let fx: Fixtures;
  let insights: NadeInsightsService;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("NadeCoachingTest");
    postgres = db.postgres;
    fx = new Fixtures(postgres, 76561199620000000n);
    insights = new NadeInsightsService(postgres, lineupsService());
  }, 600_000);
  ...
  beforeEach(async () => {
    ...
    await postgres.query(
      "UPDATE settings SET value = '96' WHERE name = 'public.nade_success_radius'",
    );
  });
  ...
  function context(...steamIds: Array<string>): NadeServerContext {
    return {
      serverId: randomUUID(),
      matchId: randomUUID(),
      mapName: "de_mirage",
  ...
  async function drilled(
    ...
    await postgres.query(
      `INSERT INTO nade_lineup_progress
         (nade_lineup_id, steam_id, attempts, successes,
  ...
  describe("decomposing a throw", () => {
    ...
    it("accumulates the offsets on the throwing player's own row", async () => {
      ...
      >(
        `SELECT miss_samples, miss_along_sum FROM nade_lineup_progress
  ...
  describe("the miss pattern", () => {
    ...
    it("calls a shared undershoot short once both floors clear", async () => {
      ...
      expect(pattern.bias).toBe(NadeInsightsService.BIAS_SHORT);
    });
    ...
    it("counts a player who threw two hundred times exactly once", async () => {
      ...
      expect(pattern.bias).toBe(NadeInsightsService.BIAS_LONG);
    });
    ...
    it("reports the axis the miss is worst on", async () => {
      ...
      expect(pattern.bias).toBe(NadeInsightsService.BIAS_RIGHT);
    });
    ...
    it("reads a high miss as high", async () => {
      ...
      expect(pattern.bias).toBe(NadeInsightsService.BIAS_HIGH);
    });
    ...
    it("says there is no pattern when everybody lands on it", async () => {
      ...
      expect(pattern.analysed).toBe(true);
      expect(pattern.bias).toBe(NadeInsightsService.BIAS_NONE);
    });
    ...
    it("refuses to name a direction the players do not agree on", async () => {
      ...
      expect(pattern.bias).toBe(NadeInsightsService.BIAS_SCATTERED);
    });
    ...
    it("scales the floor with the configured success radius", async () => {
      ...
      expect(
        (await insights.missPattern(user(author), { nade_lineup_id: lineup }))
          .bias,
      ).toBe(NadeInsightsService.BIAS_SHORT);

      await postgres.query(
        "UPDATE settings SET value = '400' WHERE name = 'public.nade_success_radius'",
      );

      expect(
        (await insights.missPattern(user(author), { nade_lineup_id: lineup }))
          .bias,
      ).toBe(NadeInsightsService.BIAS_NONE);
    });
    ...
    it("builds the pattern out of throws that landed as well as ones that did not", async () => {
      ...
      >("SELECT verified_at FROM nade_lineups WHERE id = $1::uuid", [lineup]);
    ...
    it("forgets the offsets when the lineup's geometry is edited", async () => {
      ...
      await postgres.query(
        "UPDATE nade_lineups SET land_x = land_x + 300 WHERE id = $1::uuid",
        [lineup],
      );
    ...
    it("leaves the offsets alone when the lineup is renamed", async () => {
      ...
      await postgres.query(
        "UPDATE nade_lineups SET name = 'renamed' WHERE id = $1::uuid",
        [lineup],
      );
  ...
  describe("difficulty counters", () => {
    ...
    it("does not count a player who has opened a lineup but never thrown at it", async () => {
      ...
      await postgres.query(
        `INSERT INTO nade_lineup_progress (nade_lineup_id, steam_id)
      ...
      await postgres.query(
        `UPDATE nade_lineup_progress SET attempts = 4, successes = 1
          WHERE nade_lineup_id = $1::uuid AND steam_id = $2::bigint`,
        [lineup, looker],
      );
    ...
    it("gives the counters back when a player's progress goes away", async () => {
      ...
      await postgres.query(
        "DELETE FROM nade_lineup_progress WHERE nade_lineup_id = $1::uuid AND steam_id = $2::bigint",
        [lineup, a],
      );
  ...
  describe("difficulty", () => {
    ...
    // difficulty is mounted as a computed field on nade_lineups and the web
    // app selects it in the fragment behind every nades screen, so the shape
    ...
    it("keeps the difficulty function in the shape a computed field needs", async () => {
      ...
      >(
        `SELECT p.provolatile AS volatility,
           ...
           FROM pg_proc p
          WHERE p.proname = 'nade_lineup_difficulty'
            AND p.pronamespace = 'public'::regnamespace
            AND p.proargtypes[0] = 'public.nade_lineups'::regtype`,
      );
    ...
    it("refuses to call a lineup anything off one player's afternoon", async () => {
      ...
      expect(row.difficulty).toBe(NadeInsightsService.UNMEASURED);
    });
    ...
    it("refuses to call a lineup anything off a handful of attempts", async () => {
      ...
      expect((await counters(lineup)).difficulty).toBe(
        NadeInsightsService.UNMEASURED,
      );
    });
    ...
    it("grades a measured lineup on what everybody lands", async () => {
      ...
      expect((await counters(easy)).difficulty).toBe(NadeInsightsService.EASY);
      expect((await counters(moderate)).difficulty).toBe(
        NadeInsightsService.MODERATE,
      );
      expect((await counters(hard)).difficulty).toBe(NadeInsightsService.HARD);
  ...
  describe("difficulty in the practice plan", () => {
    ...
    async function metaFor(lineupId: string, throwers: number): Promise<void> {
      await postgres.query(
        `INSERT INTO nade_meta_lineups
           (lineup_bucket, map_name, nade_type, side, technique,
            throws, throwers, matches, lineups,
            origin_x, origin_y, origin_z, land_x, land_y, land_z)
         SELECT l.lineup_bucket, l.map_name, l.nade_type, l.side, l.technique,
    ...
    it("carries the difficulty and the platform's landing rate on every entry", async () => {
      ...
      const byId = new Map(
        plan.entries.map((entry) => [entry.nade_lineup_id, entry]),
      );

      expect(byId.get(easy)?.difficulty).toBe(NadeInsightsService.EASY);
    ...
    it("leaves the existing ranking exactly where it was", async () => {
      ...
      expect(plan.entries.map((entry) => entry.nade_lineup_id)).toEqual([
        unknown,
        veryHard,
        easy,
      ]);
