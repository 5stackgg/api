...
describe("nade practice sessions (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("NadePracticeTest");
  ...
  beforeEach(async () => {
    ...
    await postgres.query(
      "DELETE FROM settings WHERE name LIKE 'public.nade_%'",
    );
  });
  ...
  async function insertSession(
    ...
    const [inserted] = await postgres.query<Array<{ id: string }>>(
      `INSERT INTO nade_practice_sessions (${cols.join(", ")})
  ...
  // The exact shape startNadePractice builds: a one-map Custom pool, no veto,
  ...
  function makeService(overrides: {
    ...
    return new NadePracticeService(
      ...
      } as unknown as never,
      {
        ...
        sendNadePracticeRefresh: jest.fn(async (): Promise<void> => undefined),
        updateMatchStatus: jest.fn(async (): Promise<void> => undefined),
        ...(overrides.matchAssistant ?? {}),
      } as unknown as never,
      new NadePlaybooksService(postgres),
      {
  ...
  describe("one live session per host", () => {
    ...
    it("refuses a Starting session while an earlier one is Ready", async () => {
      ...
      await postgres.query(
        "UPDATE nade_practice_sessions SET status = 'Ready' WHERE id = $1",
        [first],
      );
    ...
    it("frees the host once the session reaches a terminal status", async () => {
      ...
      const first = await insertSession(host);
      await postgres.query(
        "UPDATE nade_practice_sessions SET status = 'Ended' WHERE id = $1",
        [first],
      );
  ...
  describe("server headroom", () => {
    it("refuses to start when only the reserved servers are free", async () => {
      const host = await fx.player();
      await setting("public.nade_practice_enabled", "true");
      await setting("public.nade_practice_reserved_servers", "2");
      ...
      const rows = await postgres.query<Array<unknown>>(
        "SELECT 1 FROM nade_practice_sessions",
      );
    ...
    it("refuses to start at all while the feature is off", async () => {
      const host = await fx.player();
      await setting("public.nade_practice_enabled", "false");
  ...
  describe("the practice match", () => {
    ...
    it("keeps the session row when the match is swept away", async () => {
      ...
      >(
        "SELECT id::text AS id, match_id::text AS match_id FROM nade_practice_sessions WHERE id = $1",
        [sessionId],
      );
  ...
  describe("joining", () => {
    ...
    // not in that cache, so a link handed back before the nade_practice_refresh
    ...
    it("adds the lineup row and only then refreshes the server roster", async () => {
      ...
      const sendNadePracticeRefresh = jest.fn(
    ...
    it("resolves a session by its invite code", async () => {
      ...
      const [row] = await postgres.query<Array<{ invite_code: string }>>(
        "SELECT invite_code FROM nade_practice_sessions WHERE id = $1",
        [sessionId],
      );
    ...
    it("lets an invited player into a closed session", async () => {
      ...
      await postgres.query(
        `INSERT INTO nade_practice_invites
    ...
    it("refuses once the lineup is at max_players_per_lineup", async () => {
      ...
      expect(sendNadePracticeRefresh).not.toHaveBeenCalled();
      expect(await lineupCount(matchId)).toBe(10);
    });
  ...
  describe("the reaper", () => {
    ...
    it("marks an empty session's clock and leaves it alone until it is stale", async () => {
      ...
      await setting("public.nade_practice_idle_minutes", "10");
      ...
      >(
        "SELECT empty_since, status FROM nade_practice_sessions WHERE id = $1",
        [sessionId],
      );
    ...
    it("ends a session that has been empty past the idle window", async () => {
      ...
      await setting("public.nade_practice_idle_minutes", "10");
      await postgres.query(
        "UPDATE nade_practice_sessions SET empty_since = now() - interval '30 minutes' WHERE id = $1",
        [sessionId],
      );
      ...
      const [row] = await postgres.query<Array<{ status: string }>>(
        "SELECT status FROM nade_practice_sessions WHERE id = $1",
        [sessionId],
      );
    ...
    it("resets the idle clock while someone is connected", async () => {
      ...
      >(
        "SELECT status, empty_since FROM nade_practice_sessions WHERE id = $1",
        [sessionId],
      );
      expect(row.status).toBe("Ready");
    ...
    it("fails a session that never left Starting", async () => {
      ...
      const sessionId = await insertSession(host, { match_id: matchId });
      await postgres.query(
        "UPDATE nade_practice_sessions SET created_at = now() - interval '1 hour' WHERE id = $1",
        [sessionId],
      );
      ...
      >("SELECT status, expires_at FROM nade_practice_sessions WHERE id = $1", [
        sessionId,
      ]);
  ...
  describe("access and connection fields", () => {
    ...
    async function canView(
      ...
      const [row] = await postgres.query<Array<{ ok: boolean }>>(
        `SELECT can_view_nade_practice_session(s, $2::json) AS ok
           FROM nade_practice_sessions s WHERE s.id = $1`,
    ...
    // is exactly what joinNadePractice creates -- so a session with no match
    ...
    it("returns no connection link before there is a match", async () => {
      ...
      >(
        `SELECT nade_practice_connection_link(s, $2::json) AS link,
                nade_practice_connection_string(s, $2::json) AS connect
           FROM nade_practice_sessions s WHERE s.id = $1`,
    ...
    it("reports lineup membership through is_nade_practice_member", async () => {
      const host = await fx.player();
      const { matchId } = await createPracticeMatch(host);
      const sessionId = await insertSession(host, { match_id: matchId });
      const stranger = await fx.player();
      ...
      const member = async (steamId: string) => {
        const [row] = await postgres.query<Array<{ ok: boolean }>>(
          `SELECT is_nade_practice_member(s, $2::json) AS ok
             FROM nade_practice_sessions s WHERE s.id = $1`,
  ...
  describe("telling people", () => {
    it("notifies the players an invite actually reached", async () => {
      ...
      expect(notified[0].type).toBe("NadePracticeInvite");
      ...
      // board opens, not a page, and the code is what joinNadePractice takes.
      ...
      expect(notified[0].message).toContain(
        `https://5stack.test/nades/de_mirage?practice=${invite_code}`,
      );
    ...
    // The plugin polls GET /nades/session, so markReady runs over and over.
    ...
    it("announces a ready server once, to the host and the lineup", async () => {
      ...
      expect(notified).toHaveLength(1);
      expect(notified[0].type).toBe("NadePracticeReady");
      ...
      expect(notified[0].message).toContain(
        "https://5stack.test/nades/de_mirage",
      );
    });
  });
});
