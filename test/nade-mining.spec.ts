...
import {
  ...
  NadeTrajectoryInput,
} from "./../src/nades/nade-artifacts.service";
import { NadeLineupsService } from "./../src/nades/nade-lineups.service";
import { NadeMiningService } from "./../src/nades/nade-mining.service";
...
describe("nade lineups mined from demos (SQL-driven)", () => {
  let db: SqlTestDb;
  let postgres: PostgresService;
  let fx: Fixtures;
  let uploads: Array<NadeTrajectoryInput>;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("NadeMiningTest");
  ...
  function artifactsStub() {
    return {
      uploadTrajectory: jest.fn(
        async (input: NadeTrajectoryInput): Promise<string> => {
          uploads.push(input);
          return "nades/test/trajectory.json.gz";
        },
      ),
      removeTrajectories: jest.fn(async (): Promise<void> => undefined),
    } as unknown as NadeArtifactsService;
  }
  ...
  function lineupsService(artifacts: NadeArtifactsService) {
    return new NadeLineupsService(
      new Logger("NadeMiningTest"),
      postgres,
      artifacts,
  ...
  function miningService(
    ...
    blob: PlaybackBlob,
    overrides: {
      artifacts?: NadeArtifactsService;
  ...
  describe("recovering the throw from the flight", () => {
    it("derives the release aim from the trajectory to within a couple of degrees", () => {
      const mined = NadeMiningService.mine(
    ...
    it("derives it through a run, where the release velocity is not the aim", () => {
      ...
      const mined = NadeMiningService.mine(blob, 7, "de_mirage");
    ...
    it("records the disagreement with the demo's own view angles instead of picking a winner", () => {
      const agreeing = NadeMiningService.mine(
      ...
      const disagreeing = NadeMiningService.mine(
    ...
    it("refuses a grenade that never landed", () => {
      expect(() =>
        NadeMiningService.mine(blobWith({ detonated: false }), 7, "de_mirage"),
  ...
  describe("classifying the throw", () => {
    it("buckets a full, a half and a drop, and names nothing in between", () => {
      expect(NadeMiningService.strengthOf(750)).toBe("Full");
      expect(NadeMiningService.strengthOf(660)).toBe("Full");
      expect(NadeMiningService.strengthOf(420)).toBe("Half");
      expect(NadeMiningService.strengthOf(320)).toBe("Half");
      expect(NadeMiningService.strengthOf(150)).toBe("Drop");
      ...
      // nobody could see.
      expect(NadeMiningService.strengthOf(620)).toBeNull();
      expect(NadeMiningService.strengthOf(280)).toBeNull();
    });

    it("buckets the impulse rather than the ground speed it was thrown at", () => {
      const running = NadeMiningService.mine(
    ...
    it("reads the crouch flag the parser now carries", () => {
      expect(
        NadeMiningService.mine(blobWith({ ducked: true }), 7, "de_mirage")
          .technique,
      ).toBe("Crouch");
      expect(
        NadeMiningService.mine(blobWith({ ducked: false }), 7, "de_mirage")
          .technique,
      ).toBe("Stationary");
    });
  ...
  describe("the HE naming trap", () => {
    it("maps the parser's HE onto the enum's HighExplosive", () => {
      ...
      expect(mined.nadeType).toBe("HighExplosive");
    });

    it("maps every other type through unchanged", () => {
      for (const type of ["Flash", "Smoke", "Molotov", "Decoy"]) {
        expect(
          NadeMiningService.mine(blobWith({ type }), 7, "de_mirage").nadeType,
        ).toBe(type);
      }
    });
    ...
    it("refuses a type it has no name for rather than dropping the lineup", () => {
      expect(() =>
        NadeMiningService.mine(blobWith({ type: "Nuke" }), 7, "de_mirage"),
    ...
    it("stores an HE lineup, which the utility-type foreign key would reject unmapped", async () => {
      ...
      const [row] = await postgres.query<Array<{ nade_type: string }>>(
        "SELECT nade_type FROM nade_lineups WHERE id = $1::uuid",
        [id],
      );
      expect(row.nade_type).toBe("HighExplosive");
    });
  });
  ...
  describe("writing the lineup", () => {
    ...
    it("refuses to mine a match Hasura will not show the caller", async () => {
      ...
      const [row] = await postgres.query<Array<{ count: string }>>(
        "SELECT COUNT(*) AS count FROM nade_lineups",
      );
  ...
  describe("the smoke bloom artifact", () => {
    ...
    function artifactsWith(smokeVolume: SmokeVolumeMock) {
      ...
      const service = new NadeArtifactsService(
        new Logger("NadeMiningTest"),
        {
    ...
    const input = (nadeType: string): NadeTrajectoryInput => ({
      lineupId: randomUUID(),
      mapName: "de_mirage",
      nadeType,
      throwerSteamId: null,
  ...
  describe("post-match utility report", () => {
    ...
    async function markMined(
      ...
      await postgres.query(
        `INSERT INTO nade_demo_mines (match_map_demo_id, version, throws, failed_reason)
         VALUES ($1::uuid, 1, 1, $2)`,
        [demoId, failedReason],
      );
    }
    ...
    async function insertThrow(
      ...
      await postgres.query(
        `INSERT INTO nade_demo_throws (${cols.join(", ")})
    ...
    it("counts throws, the lineups they match and where they landed", async () => {
      ...
      expect(answer.radius).toBe(NadeLineupsService.DEFAULT_SUCCESS_RADIUS);
    ...
    it("counts a mined meta cluster even with nothing written up", async () => {
      ...
      >("SELECT lineup_bucket FROM nade_demo_throws LIMIT 1");
