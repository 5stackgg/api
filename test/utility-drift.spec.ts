...
import { UtilityDriftService } from "./../src/utility/utility-drift.service";
...
describe("utility drift scans (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("UtilityDriftTest");
  ...
  function service(parser: DemoParserService) {
    return new UtilityDriftService(
      new Logger("UtilityDriftTest"),
      postgres,
      parser,
      {
        send: jest.fn(
  ...
  const scanRow = async (scanId: string) => {
    ...
    >("SELECT * FROM utility_drift_scans WHERE id = $1::uuid", [scanId]);
    return row;
  };
  ...
  describe("running a scan", () => {
    it("persists a verdict per lineup, unsimulatable included", async () => {
      ...
      >(
        `SELECT utility_lineup_id::text AS utility_lineup_id, verdict, severity, distance
           FROM utility_drift_results WHERE utility_drift_scan_id = $1::uuid`,
        [scan.scan_id],
      );

      const verdicts = new Map(
        rows.map((row) => [row.utility_lineup_id, row.verdict]),
      );
    ...
    it("translates the panel's grenade spelling into the simulator's", async () => {
      const author = await fx.player();
      await insertLineup(author, { utility_type: "HighExplosive" });
      await insertLineup(author, { utility_type: "Smoke" });
      ...
      const types = driftCalls[0].lineups.map((lineup) => lineup.utility_type);
    ...
    it("stores no coordinate, only the gap between the two runs", async () => {
      ...
      const columns = await postgres.query<Array<{ column_name: string }>>(
        `SELECT column_name FROM information_schema.columns
          WHERE table_schema = 'public' AND table_name = 'utility_drift_results'
      ...
      // reads as "where your utility lands".
  ...
  describe("announcing the outcome", () => {
    it("tells administrators the headline counts", async () => {
      ...
      expect(sent[0].type).toBe("UtilityDriftScanFinished");
      ...
      expect(sent[0].message).toContain("https://5stack.test/utility/drift");
    });
