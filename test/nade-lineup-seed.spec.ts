...
describe("nade lineup physics seed (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("NadeSeedTest");
  ...
  it("refuses a velocity faster than the engine allows", async () => {
    ...
    await expect(
      service.ingest(
        ...
        payload(author, {
          ...SEED,
          initial_vel_x: NadeLineupsService.MAX_VELOCITY + 1,
          initial_vel_y: 0,
          initial_vel_z: 0,
        }),
      ),
  ...
  it("hands the seed to the trajectory artifact alongside the path", async () => {
    ...
    const blob = NadeArtifactsService.buildTrajectoryBlob(uploads[0]);
