...
import { NadeAnalysisService } from "./../src/nades/nade-analysis.service";
import { NadeArtifactsService } from "./../src/nades/nade-artifacts.service";
...
describe("nade analysis (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("NadeAnalysisTest");
  ...
  function service(
    ...
    return new NadeAnalysisService(
      new Logger("NadeAnalysisTest"),
      postgres,
  ...
  describe("degradation", () => {
    it("answers a degraded sightline result when the parser is down", async () => {
      ...
      expect(answer.threshold).toBe(NadeAnalysisService.DEFAULT_THRESHOLD);
    });
  ...
  describe("the blocking search", () => {
    ...
    it("only considers smokes near the segment", async () => {
      ...
      expect(answer.results.map((result) => result.nade_lineup_id)).toEqual([
        near,
      ]);
    });
    ...
    it("never evaluates more candidates than the parser accepts clouds", async () => {
      ...
      const answer = await service(
        parserStub({ sightlines: clear(NadeAnalysisService.CANDIDATE_CAP) }),
      ).findBlocking(user(author), { ...line, limit: 50 });

      expect(sightlineCalls).toHaveLength(1);
      expect(sightlineCalls[0].smokes).toHaveLength(
        NadeAnalysisService.CANDIDATE_CAP,
      );
      expect(answer.results).toHaveLength(NadeAnalysisService.CANDIDATE_CAP);
    });
    ...
    it("prefers the bloom the artifact already carries", async () => {
      ...
      await insertLineup(author, {
        ...
        land_y: 0,
        land_z: 0,
        trajectory_file: "nades/abc/trajectory.json.gz",
      });
      ...
      expect(artifactCalls).toEqual(["nades/abc/trajectory.json.gz"]);
  ...
  describe("playbook coverage", () => {
    async function insertPlaybook(
      ...
      const [playbook] = await postgres.query<Array<{ id: string }>>(
        `INSERT INTO nade_playbooks (name, map_name, side, owner_steam_id, visibility)
      ...
      for (const [order, lineupId] of lineupIds.entries()) {
        await postgres.query(
          `INSERT INTO nade_playbook_steps (playbook_id, nade_lineup_id, step_order, offset_ms)
           VALUES ($1, $2, $3, $4)`,
    ...
    it("degrades when the book holds more smokes than one request carries", async () => {
      ...
      for (let index = 0; index <= NadeAnalysisService.CANDIDATE_CAP; index++) {
    ...
    it("says a book with no smokes covers nothing, without asking", async () => {
      const author = await fx.player();
      const flash = await insertLineup(author, { nade_type: "Flash" });
