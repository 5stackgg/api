...
describe("nade practice servers (SQL-driven)", () => {
  let db: SqlTestDb;
  let postgres: PostgresService;
  let fx: Fixtures;

  beforeAll(async () => {
    db = await bootMigratedDb("NadePracticeServersTest");
  ...
  beforeEach(async () => {
    ...
    await postgres.query("DELETE FROM settings WHERE name LIKE 'public.nade_%'");
  });
  ...
  function makeService(): NadePracticeService {
    return new NadePracticeService(
      new Logger("NadePracticeServersTest"),
      postgres,
      {} as never,
      {
