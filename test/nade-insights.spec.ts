...
// out of nade_demo_throws, which names players and is admin-only for that
// reason.
describe("nade insights (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("NadeInsightsTest");
    postgres = db.postgres;
    fx = new Fixtures(postgres, 76561199640000000n);

    const lineups = new NadeLineupsService(
      new Logger("NadeInsightsTest"),
      postgres,
      {} as unknown as never,
      {} as unknown as never,
    );

    insights = new NadeInsightsService(postgres, lineups);
  ...
  describe("the practice plan", () => {
    ...
    it("only plans lineups the caller can see", async () => {
      ...
      expect(plan.entries.map((entry) => entry.nade_lineup_id)).toEqual([mine]);
    });
    ...
    it("offers one lineup per meta bucket", async () => {
      ...
      expect(plan.entries[0].nade_lineup_id).toBe(best);
    });
  ...
  describe("the team utility report", () => {
    ...
    async function throwOf(
      ...
      await postgres.query(
        `INSERT INTO nade_demo_throws
           (match_map_demo_id, grenade_id, map_name, nade_type, side, technique,
    ...
    it("counts how often the roster threw it and how often it landed", async () => {
      ...
      expect(report.entries).toEqual([
        { nade_lineup_id: lineup, thrown: 12, landed: 9, players: 3 },
      ]);
    });

    // nade_demo_throws names a player and the match they threw in. Nothing
    ...
    it("ignores throws by people who are not on the roster", async () => {
      ...
      expect(report.entries).toEqual([
        { nade_lineup_id: lineup, thrown: 1, landed: 1, players: 1 },
      ]);
    });
    ...
    it("scopes to one map when asked", async () => {
      ...
      expect(report.entries).toEqual([
        { nade_lineup_id: onNuke, thrown: 1, landed: 1, players: 1 },
      ]);
    });
  });
});
