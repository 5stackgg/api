...
describe("utility lineup verification (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("UtilityVerifyTest");
  ...
  beforeEach(async () => {
    ...
    await postgres.query(
      "UPDATE settings SET value = '3' WHERE name = 'public.utility_verify_masteries'",
    );
  });
  ...
  async function verifiedAt(lineupId: string): Promise<Date | null> {
    const [row] = await postgres.query<Array<{ verified_at: Date | null }>>(
      "SELECT verified_at FROM utility_lineups WHERE id = $1::uuid",
      [lineupId],
    );
  ...
  describe("deriving verification", () => {
    ...
    it("counts players, not masteries", async () => {
      ...
      for (let attempt = 0; attempt < 5; attempt++) {
        await postgres.query(
          `UPDATE utility_lineup_progress
    ...
    it("reads the bar out of the setting", async () => {
      await setting("public.utility_verify_masteries", "2");
    ...
    it("falls back rather than failing the write on a nonsense setting", async () => {
      await setting("public.utility_verify_masteries", "soon");
