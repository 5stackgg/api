...
import { UtilityMetaService } from "./../src/utility/utility-meta.service";
...
describe("utility meta clustering (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("UtilityMetaTest");
  ...
  async function metaFor(blob: PlaybackBlob | null): Promise<UtilityMetaService> {
    ...
    return new UtilityMetaService(
      new Logger("UtilityMetaTest"),
      postgres,
      {
  ...
  const clusters = async () =>
    ...
    >(
      `SELECT lineup_bucket, map_name, utility_type, technique, throws, throwers,
              matches, lineups, origin_x, land_x
         FROM utility_meta_lineups
  ...
  it("counts people, not throws, for a lineup several players run", async () => {
    ...
    expect(rows[0].utility_type).toBe("Smoke");
  });
  ...
  it("counts a smoke and an HE from the same spot as different lineups", async () => {
    ...
    const rows = await clusters();
    expect(rows.length).toBe(2);
    expect(rows.map((row) => row.utility_type).sort()).toEqual([
      "HighExplosive",
      "Smoke",
    ]);
  });
  ...
  it("shares its bucket with a saved lineup at the same spot", async () => {
    ...
    const [saved] = await postgres.query<Array<{ lineup_bucket: string }>>(
      "SELECT lineup_bucket FROM utility_lineups LIMIT 1",
    );
  ...
  it("records why a demo could not be mined and carries on with the rest", async () => {
    ...
    >("SELECT failed_reason, throws FROM utility_demo_mines");
  ...
  it("stores a throw whose strength it could not name", async () => {
    ...
    >("SELECT throw_strength FROM utility_demo_throws");
