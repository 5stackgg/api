...
// (POST /nades/ingest). So the only things worth pinning here are the refusals:
...
describe("nade solver (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("NadeSolverTest");
  ...
  function service(rcon: RconService, cache?: CacheService) {
    return new NadeSolverService(
      new Logger("NadeSolverTest"),
      postgres,
  ...
  describe("guards", () => {
    ...
    it("refuses a session that is not live", async () => {
      ...
      expect(answer.accepted).toBe(false);
      expect(answer.status).toBe(NadeSolverStatus.NotLive);
    ...
    it("refuses a session with no server", async () => {
      ...
      expect(answer.accepted).toBe(false);
      expect(answer.status).toBe(NadeSolverStatus.NoServer);
    });
  ...
  describe("the calibration gate", () => {
    it("refuses, with the plugin's own reason, when calibration is not ready", async () => {
      ...
      expect(commands).toEqual(["nade_solver_calibrate"]);
    });
    ...
    it("answers Unsupported on CounterStrikeSharp without touching rcon", async () => {
      ...
      expect(answer.status).toBe(NadeSolverStatus.Unsupported);
    ...
    it("reports an unreachable server rather than failing", async () => {
      ...
      expect(answer.status).toBe(NadeSolverStatus.Unreachable);
      expect(answer.ready).toBe(false);
    ...
    it("reads a cached ready verdict", () => {
      expect(NadeSolverService.readCalibration(READY)).toEqual({
        status: NadeSolverStatus.Ready,
        ready: true,
    ...
    it("does not mistake a busy solver for a verdict", () => {
      const answer = NadeSolverService.readCalibration(
        " \x02solve is already running",
      );

      expect(answer.status).toBe(NadeSolverStatus.Busy);
      expect(answer.ready).toBe(false);
    });
    ...
    it("treats an in-progress calibration as no verdict yet", () => {
      ...
      expect(answer.status).toBe(NadeSolverStatus.Unknown);
      expect(answer.ready).toBe(false);
    });
  });
  ...
  describe("issuing the solve", () => {
    it("accepts and returns without waiting for a lineup", async () => {
      ...
      expect(commands[1]).toBe(
        `nade_solver_solve target=100.00,200.00,30.00 from=-10.00,-20.00,-30.00 utility=Smoke steam=${host} tolerance=40.00`,
      );
    });
