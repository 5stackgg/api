...
describe("nade lineup forks (SQL-driven)", () => {
  let db: SqlTestDb;
  let postgres: PostgresService;
  let fx: Fixtures;
  let lineups: NadeLineupsService;

  beforeAll(async () => {
    db = await bootMigratedDb("NadeForksTest");
    postgres = db.postgres;
    fx = new Fixtures(postgres, 76561199610000000n);
    lineups = new NadeLineupsService(
      new Logger("NadeForksTest"),
      postgres,
      {
  ...
  const rowOf = async (lineupId: string) => {
    const [row] = await postgres.query<Array<Record<string, unknown>>>(
      "SELECT * FROM nade_lineups WHERE id = $1",
      [lineupId],
    );
    return row;
  };
  ...
  describe("what crosses", () => {
    ...
    it("lands private, owned by the forker, pointing at its source", async () => {
      ...
      expect(String(fork.forked_from_nade_lineup_id)).toBe(source);
    });
  ...
  describe("what does not cross", () => {
    it("leaves the votes, favourites and progress behind", async () => {
      ...
      await postgres.query(
        `INSERT INTO nade_lineup_progress (nade_lineup_id, steam_id, attempts, successes)
         VALUES ($1, $2, 12, 9)`,
        [source, voter],
      );
      ...
      const [{ count }] = await postgres.query<Array<{ count: string }>>(
        `SELECT
           (SELECT COUNT(*) FROM nade_lineup_votes WHERE nade_lineup_id = $1) +
           (SELECT COUNT(*) FROM nade_lineup_favorites WHERE nade_lineup_id = $1) +
           (SELECT COUNT(*) FROM nade_lineup_progress WHERE nade_lineup_id = $1) AS count`,
        [id],
      );
    ...
    it("drops the source match, grenade and external id", async () => {
      ...
      const source = await insertLineup(author, {
        ...
        source_url: "https://example.test/nade",
        external_id: "abc-123",
    ...
    it("does not point at the original's trajectory in S3", async () => {
      ...
      const forker = await fx.player();
      const source = await insertLineup(author, {
        trajectory_file: "nades/de_mirage/original.json.gz",
    ...
    it("survives the original being deleted", async () => {
      ...
      await postgres.query("DELETE FROM nade_lineups WHERE id = $1", [source]);

      const fork = await rowOf(id);
      expect(fork).toBeTruthy();
      expect(fork.forked_from_nade_lineup_id).toBeNull();
  ...
  describe("visibility", () => {
    it("refuses a lineup the caller cannot see", async () => {
      ...
      await expect(
        lineups.fork(user(stranger), { nade_lineup_id: source }),
      ).rejects.toThrow("lineup not found");

      const [{ count }] = await postgres.query<Array<{ count: string }>>(
        "SELECT COUNT(*) AS count FROM nade_lineups WHERE author_steam_id = $1",
        [stranger],
      );
      expect(Number(count)).toBe(0);
    });
    ...
    it("refuses a lineup id that is not a lineup", async () => {
      ...
      await expect(
        lineups.fork(user(forker), { nade_lineup_id: "not-a-uuid" }),
      ).rejects.toThrow("lineup not found");
    });
  });
  ...
  describe("collections", () => {
    it("files the fork into a collection the caller owns", async () => {
      ...
      const [{ count }] = await postgres.query<Array<{ count: string }>>(
        `SELECT COUNT(*) AS count FROM nade_collection_items
          WHERE collection_id = $1 AND nade_lineup_id = $2`,
        [collection.id, id],
      );
