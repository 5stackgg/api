...
// been told anything.
describe("utility playbooks (SQL-driven)", () => {
  let db: SqlTestDb;
  let postgres: PostgresService;
  let fx: Fixtures;
  let playbooks: UtilityPlaybooksService;

  beforeAll(async () => {
    db = await bootMigratedDb("UtilityPlaybooksTest");
    postgres = db.postgres;
    fx = new Fixtures(postgres, 76561199900000000n);
    playbooks = new UtilityPlaybooksService(postgres);
  ...
  async function insertPlaybook(
    ...
    const cols = Object.keys(row);
    const [inserted] = await postgres.query<Array<{ id: string }>>(
      `INSERT INTO utility_playbooks (${cols.join(", ")})
  ...
  async function insertStep(
    ...
    const cols = Object.keys(row);
    const [inserted] = await postgres.query<Array<{ id: string }>>(
      `INSERT INTO utility_playbook_steps (${cols.join(", ")})
       VALUES (${cols.map((_, i) => `$${i + 1}`).join(", ")})
       RETURNING id::text AS id`,
      Object.values(row) as Array<string>,
    );
  ...
  async function canView(
    ...
    const [row] = await postgres.query<Array<{ ok: boolean }>>(
      `SELECT can_view_utility_playbook(p, $2::json) AS ok
         FROM utility_playbooks p WHERE p.id = $1::uuid`,
  ...
  async function canEdit(
    ...
    const [row] = await postgres.query<Array<{ ok: boolean }>>(
      `SELECT can_edit_utility_playbook(p, $2::json) AS ok
         FROM utility_playbooks p WHERE p.id = $1::uuid`,
      [playbookId, session(viewer, role)],
    );
    return row.ok;
  ...
  async function stepRows(playbookId: string) {
    ...
    >(
      `SELECT step_order, offset_ms, utility_lineup_id::text AS utility_lineup_id,
              assigned_steam_id::text AS assigned_steam_id, note
         FROM utility_playbook_steps
  ...
  describe("visibility", () => {
    ...
    it("refuses Team visibility without a team", async () => {
      ...
      ).rejects.toThrow(/utility_playbooks_team_scope_chk/);
    });
  ...
  // trigger, so without the demotion in tbiu_utility_playbooks a team that had
  ...
  describe("when the team goes away", () => {
    it("demotes a team playbook to private instead of blocking the delete", async () => {
      ...
      >(
        "SELECT visibility, team_id::text AS team_id FROM utility_playbooks WHERE id = $1::uuid",
        [id],
      );
  ...
  describe("steps", () => {
    it("refuses two steps in the same slot", async () => {
      ...
      ).rejects.toThrow(/utility_playbook_steps_order_key/);
    });
    ...
    it("allows an in-place reorder inside one transaction", async () => {
      ...
      expect(steps.map((step) => step.utility_lineup_id)).toEqual([second, first]);
    });
    ...
    it("refuses moving a written playbook to another map", async () => {
      ...
      await expect(
        postgres.query(
          "UPDATE utility_playbooks SET map_name = 'de_nuke' WHERE id = $1::uuid",
          [playbook],
        ),
    ...
    it("refuses an offset outside the execute", async () => {
      ...
      ).rejects.toThrow(/utility_playbook_steps_offset_chk/);
    });
    ...
    it("cascades steps when the playbook or the lineup goes away", async () => {
      ...
      await postgres.query("DELETE FROM utility_lineups WHERE id = $1", [lineup]);
      expect((await stepRows(playbook)).length).toBe(1);

      await postgres.query("DELETE FROM utility_playbooks WHERE id = $1::uuid", [
        playbook,
      ]);
  ...
  describe("saving a playbook", () => {
    it("creates a book whose steps are ordered by the order they arrived", async () => {
      ...
      const { id } = await playbooks.save(asUser(owner), {
        ...
        side: "TERRORIST",
        steps: [
          { utility_lineup_id: smoke, offset_ms: 0, note: "CT smoke" },
          {
            utility_lineup_id: flash,
            offset_ms: 2500,
      ...
      expect(steps.map((step) => step.utility_lineup_id)).toEqual([smoke, flash]);
    ...
    it("replaces the steps on update, and leaves them alone when none are sent", async () => {
      ...
      const { id } = await playbooks.save(asUser(owner), {
        ...
        map_name: "de_mirage",
        side: "TERRORIST",
        steps: [{ utility_lineup_id: smoke }, { utility_lineup_id: flash }],
      });
      ...
      await playbooks.save(asUser(owner), {
        ...
        map_name: "de_mirage",
        side: "TERRORIST",
        steps: [{ utility_lineup_id: flash, offset_ms: 1000 }],
      });

      let steps = await stepRows(id);
      expect(steps.length).toBe(1);
      expect(steps[0].utility_lineup_id).toBe(flash);
      ...
      const [row] = await postgres.query<Array<{ name: string }>>(
        "SELECT name FROM utility_playbooks WHERE id = $1::uuid",
        [id],
      );
    ...
    it("refuses a step pointing at a lineup the author cannot see", async () => {
      ...
      await expect(
        playbooks.save(asUser(owner), {
          ...
          map_name: "de_mirage",
          side: "TERRORIST",
          steps: [{ utility_lineup_id: secret }],
        }),
      ).rejects.toThrow(/does not exist/);

      const rows = await postgres.query<Array<unknown>>(
        "SELECT 1 FROM utility_playbooks",
      );
    ...
    it("refuses a step on another map", async () => {
      ...
      await expect(
        playbooks.save(asUser(owner), {
          ...
          map_name: "de_mirage",
          side: "TERRORIST",
          steps: [{ utility_lineup_id: elsewhere }],
        }),
    ...
    it("refuses more steps than an execute can hold", async () => {
      ...
      await expect(
        playbooks.save(asUser(owner), {
          ...
          side: "TERRORIST",
          steps: Array.from(
            { length: UtilityPlaybooksService.MAX_STEPS + 1 },
      ...
      await expect(
        playbooks.save(asUser(owner), {
          ...
          steps: [
            {
              utility_lineup_id: lineup,
              offset_ms: UtilityPlaybooksService.MAX_OFFSET_MS + 1,
            },
          ],
        }),
    ...
    it("refuses an assignment to somebody who is not a player", async () => {
      ...
      await expect(
        playbooks.save(asUser(owner), {
          ...
          side: "TERRORIST",
          steps: [
            { utility_lineup_id: lineup, assigned_steam_id: "76561199999999999" },
          ],
        }),
  ...
  describe("deleting a playbook", () => {
    ...
    it("clears the book out of any session running it", async () => {
      ...
      const [row] = await postgres.query<Array<{ playbook_id: string | null }>>(
        "SELECT playbook_id::text AS playbook_id FROM utility_practice_sessions WHERE id = $1::uuid",
        [inserted.id],
      );
  ...
  describe("loading a playbook into a session", () => {
    function makePractice(overrides: {
      ...
      return new UtilityPracticeService(
        new Logger("UtilityPlaybooksTest"),
        postgres,
        {} as never,
        {
        ...
        } as unknown as never,
        {
          ...
          sendUtilityPracticeRefresh: jest.fn(
    ...
    it("writes the playbook and only then refreshes the server", async () => {
      ...
      const sendUtilityPracticeRefresh = jest.fn(async (): Promise<void> => {
    ...
    it("hands the server the ordered execute with its lineups", async () => {
      ...
      await postgres.query(
        "UPDATE utility_practice_sessions SET playbook_id = $2::uuid WHERE id = $1::uuid",
        [sessionId, playbook],
      );
      ...
      expect(
        payload?.playbook?.steps.map((step) => step.utility_lineup_id),
      ).toEqual([smoke, flash]);
      ...
      expect(payload?.playbook?.steps[1].lineup.utility_type).toBe("Flash");
    });
