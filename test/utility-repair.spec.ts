...
import { UtilityRepairService } from "./../src/utility/utility-repair.service";
...
describe("utility lineup repair (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  beforeAll(async () => {
    db = await bootMigratedDb("UtilityRepairTest");
  ...
  beforeEach(async () => {
    commands = [];
    await postgres.query("DELETE FROM utility_lineup_repairs");
    await postgres.query("DELETE FROM utility_drift_scans");
  ...
  function service(
    ...
    return new UtilityRepairService(
      new Logger("UtilityRepairTest"),
      postgres,
      new UtilitySolverService(
        new Logger("UtilityRepairTest"),
        postgres,
        solverCache(),
        rcon(replies),
  ...
  async function verdictFor(
    ...
    const [scan] = await postgres.query<Array<{ id: string }>>(
      `INSERT INTO utility_drift_scans (map_name, requested_by_steam_id)
       VALUES ('de_mirage', $1::bigint) RETURNING id`,
      [requester],
    );
    await postgres.query(
      `INSERT INTO utility_drift_results
         (utility_drift_scan_id, utility_lineup_id, verdict, severity, distance)
  ...
  const repairRows = async () =>
    await postgres.query<
      Array<{
        ...
        status: string;
        repaired_utility_lineup_id: string | null;
        drift_distance: number | null;
        repaired_at: Date | null;
      }>
    >(
      `SELECT id::text AS id, utility_lineup_id::text AS utility_lineup_id, status,
              repaired_utility_lineup_id::text AS repaired_utility_lineup_id,
              drift_distance, repaired_at
         FROM utility_lineup_repairs ORDER BY created_at ASC`,
    );
  ...
  describe("refusing what a re-solve cannot fix", () => {
    it("refuses a lineup no scan has judged", async () => {
      ...
      expect(answer.status).toBe(UtilitySolverStatus.NotScanned);
    ...
    it.each([
      ...
      expect(answer.accepted).toBe(false);
      expect(answer.status).toBe(UtilitySolverStatus.NotMoved);
    ...
    it("refuses a lineup whose seed has gone", async () => {
      ...
      expect(answer.accepted).toBe(false);
      expect(answer.status).toBe(UtilitySolverStatus.Seedless);
      expect(commands).toHaveLength(0);
    });
    ...
    it("refuses to aim one map's coordinates at another map", async () => {
      ...
      expect(answer.accepted).toBe(false);
      expect(answer.status).toBe(UtilitySolverStatus.WrongMap);
      expect(commands).toHaveLength(0);
    });
  ...
  describe("issuing the re-solve", () => {
    it("aims at the drifted lineup's own landing point, from its own stance", async () => {
      ...
      expect(repair.utility_lineup_id).toBe(lineup);
  ...
  describe("when the solved lineup arrives", () => {
    ...
    it("links the new lineup back to the one it repaired", async () => {
      ...
      >(
        `SELECT forked_from_utility_lineup_id::text AS forked_from, name,
      ...
      expect(closed.repaired_utility_lineup_id).toBe(fixed);
    ...
    it("destroys nothing the original had earned", async () => {
      ...
      await postgres.query(
        "INSERT INTO utility_lineup_votes (utility_lineup_id, steam_id, vote) VALUES ($1::uuid, $2::bigint, 1)",
        [original, fan],
      );
      await postgres.query(
        "INSERT INTO utility_lineup_favorites (utility_lineup_id, steam_id) VALUES ($1::uuid, $2::bigint)",
        [original, fan],
      );
      await postgres.query(
      ...
      >(
        `SELECT l.land_x, l.upvotes, l.favorites,
                (SELECT count(*)::text FROM utility_lineup_progress p
                  WHERE p.utility_lineup_id = l.id) AS progress,
                (SELECT count(*)::text FROM utility_drift_results d
                  WHERE d.utility_lineup_id = l.id AND d.verdict = 'moved') AS verdicts
    ...
    it("does not claim an ordinary lineup saved in the same session", async () => {
      ...
      const [row] = await postgres.query<Array<{ forked_from: string | null }>>(
        `SELECT forked_from_utility_lineup_id::text AS forked_from
           FROM utility_lineups WHERE id = $1::uuid`,
        [unrelated],
      );
    ...
    it("will not claim a repair the window has closed on", async () => {
      ...
      await postgres.query(
        "UPDATE utility_lineup_repairs SET expires_at = now() - interval '1 minute' WHERE id = $1::uuid",
        [repair.id],
      );
