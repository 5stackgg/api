...
// The utility library's guarantees, run as real SQL against a migrated database.
...
describe("utility lineups (SQL-driven)", () => {
  let db: SqlTestDb;
  let postgres: PostgresService;
  let fx: Fixtures;

  beforeAll(async () => {
    db = await bootMigratedDb("UtilityLineupsTest");
  ...
  async function canView(
    ...
    const [row] = await postgres.query<Array<{ ok: boolean }>>(
      `SELECT can_view_utility_lineup(l, $2::json) AS ok
         FROM utility_lineups l WHERE l.id = $1`,
  ...
  async function canEdit(
    ...
    const [row] = await postgres.query<Array<{ ok: boolean }>>(
      `SELECT can_edit_utility_lineup(l, $2::json) AS ok
         FROM utility_lineups l WHERE l.id = $1`,
      [lineupId, session(viewer, role)],
    );
    return row.ok;
  ...
  describe("visibility", () => {
    ...
    it("refuses Team visibility without a team", async () => {
      ...
      ).rejects.toThrow(/utility_lineups_team_scope_chk/);
    });
  ...
  describe("counters", () => {
    it("tracks up and down votes", async () => {
      ...
      >("SELECT upvotes, downvotes FROM utility_lineups WHERE id = $1", [id]);
      expect(row.upvotes).toBe(1);
      expect(row.downvotes).toBe(1);

      // Flipping a vote must move both counters, not just add to one.
      await postgres.query(
        "UPDATE utility_lineup_votes SET vote = 1 WHERE utility_lineup_id = $1 AND steam_id = $2",
        [id, b],
      );
      ...
      await postgres.query(
        "DELETE FROM utility_lineup_votes WHERE utility_lineup_id = $1",
        [id],
      );
      [row] = await postgres.query<
        Array<{ upvotes: number; downvotes: number }>
    ...
    it("tracks favorites", async () => {
      ...
      let [row] = await postgres.query<Array<{ favorites: number }>>(
        "SELECT favorites FROM utility_lineups WHERE id = $1",
        [id],
      );
      expect(row.favorites).toBe(1);

      await postgres.query(
        "DELETE FROM utility_lineup_favorites WHERE utility_lineup_id = $1 AND steam_id = $2",
        [id, fan],
      );
    ...
    it("reports the viewer's own vote and favorite state", async () => {
      ...
      >(
        `SELECT utility_lineup_my_vote(l, $2::json) AS my_vote,
                utility_lineup_is_favorited(l, $2::json) AS is_favorited
           FROM utility_lineups l WHERE l.id = $1`,
        [id, session(fan)],
      );
  ...
  describe("dedupe bucket", () => {
    ...
    it("buckets near-identical setups together and distinct ones apart", async () => {
      ...
      >("SELECT id, lineup_bucket FROM utility_lineups WHERE id = ANY($1)", [
        [a, b, c],
      ]);
  ...
  describe("team deletion", () => {
    it("demotes a team lineup to private instead of blocking the delete", async () => {
      ...
      >("SELECT visibility, team_id FROM utility_lineups WHERE id = $1", [id]);
    ...
    it("demotes a team collection the same way", async () => {
      ...
      const [collection] = await postgres.query<Array<{ id: string }>>(
        `INSERT INTO utility_collections (name, owner_steam_id, team_id, visibility)
      ...
      const [row] = await postgres.query<Array<{ visibility: string }>>(
        "SELECT visibility FROM utility_collections WHERE id = $1",
        [collection.id],
      );
      expect(row.visibility).toBe("Private");
    });
  ...
  describe("confidence", () => {
    ...
    const confidenceOf = async (lineupId: string) => {
      ...
      >("SELECT confidence, origin_source FROM utility_lineups WHERE id = $1", [
        lineupId,
      ]);
      return row;
    };
    ...
    it("demotes when the seed is taken away", async () => {
      ...
      await postgres.query(
        "UPDATE utility_lineups SET initial_vel_z = NULL WHERE id = $1",
        [lineup],
      );
