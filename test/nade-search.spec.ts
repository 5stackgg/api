...
// Nade lineups in the global search bar. There is exactly one thing here that
...
describe("nade lineup search index (SQL-driven)", () => {
  let db: SqlTestDb;
  ...
  describe("what is indexable", () => {
    it("takes a public lineup with its author and tags", async () => {
      ...
      const [row] = await typeSense.searchableNadeLineups();

      expect(row.id).toBe(lineupId);
      expect(TypeSenseService.nadeLineupDocument(row)).toEqual({
        id: lineupId,
        name: "Window from T spawn",
        map_name: "de_mirage",
        nade_type: "Smoke",
    ...
    it("never takes a private one", async () => {
      ...
      expect(await typeSense.searchableNadeLineups()).toEqual([]);
    });
    ...
    it("pages through the library without repeating itself", async () => {
      ...
      const first = await typeSense.searchableNadeLineups({ limit: 2 });
      const second = await typeSense.searchableNadeLineups({
        limit: 2,
        after: first.at(-1)!.id,
      });
  ...
  describe("keeping the index honest", () => {
    it("indexes a public lineup", async () => {
      ...
      await typeSense.updateNadeLineup(lineupId);
    ...
    it("removes a lineup that has just gone private", async () => {
      ...
      await postgres.query(
        "UPDATE nade_lineups SET visibility = 'Private' WHERE id = $1",
        [lineupId],
      );
      await typeSense.updateNadeLineup(lineupId);
    ...
    it("removes a lineup that has just been archived", async () => {
      ...
      await postgres.query(
        "UPDATE nade_lineups SET archived_at = now() WHERE id = $1",
        [lineupId],
      );
      await typeSense.updateNadeLineup(lineupId);
    ...
    it("removes a lineup that no longer exists", async () => {
      ...
      await postgres.query("DELETE FROM nade_lineups WHERE id = $1", [
        lineupId,
      ]);
      await typeSense.updateNadeLineup(lineupId);
    ...
    it("rebuilds the whole index from the public lineups only", async () => {
      ...
      const indexed = await typeSense.reindexNadeLineups();
