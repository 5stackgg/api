-- Recreate v_team_reputation from match data (no team_scrim_outcomes table).
-- Lives in a migration because the views apply-step skips unchanged files and
-- cannot be relied on to restore a view a prior migration dropped.
CREATE OR REPLACE VIEW public.v_team_reputation AS
WITH scrim_matches AS (
    SELECT r.match_id, m.status, m.lineup_1_id, m.lineup_2_id,
