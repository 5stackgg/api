...
ALTER TABLE public.clip_render_jobs
  ADD COLUMN IF NOT EXISTS match_map_demo_id uuid;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'clip_render_jobs_match_map_demo_id_fkey'
  ) THEN
    ALTER TABLE public.clip_render_jobs
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'match_clips_match_map_demo_id_fkey'
  ) THEN
    ALTER TABLE public.match_clips
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'match_demo_sessions_match_map_demo_id_fkey'
  ) THEN
    ALTER TABLE public.match_demo_sessions
...
CREATE INDEX IF NOT EXISTS clip_render_jobs_match_map_demo_id_idx
  ON public.clip_render_jobs (match_map_demo_id);

CREATE INDEX IF NOT EXISTS match_clips_match_map_demo_id_idx
  ON public.match_clips (match_map_demo_id);

CREATE INDEX IF NOT EXISTS match_demo_sessions_match_map_demo_id_idx
