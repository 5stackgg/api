...
-- Backfill only if get_player_elo_for_match already exists. On a fresh install
-- the function hasn't been applied yet from hasura/functions/, but there are
-- also no player_elo rows to backfill, so skipping is safe.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
    ...
    WHERE p.proname = 'get_player_elo_for_match'
