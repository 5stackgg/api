CREATE OR REPLACE VIEW public.v_event_player_stats AS
-- NOT MATERIALIZED: even though e_matches is referenced 3 times below, force inlining
-- so the outer per-event filter (event_id = $1) pushes down into the event_tournaments
...
kd_agg AS (
    ...
    GROUP BY em.event_id, e.steam_id
),
assists_agg AS (
    SELECT
        em.event_id,
    ...
    FROM e_matches em
    JOIN player_assists pa ON pa.match_id = em.match_id
    WHERE pa.attacker_steam_id IS NOT NULL
    GROUP BY em.event_id, pa.attacker_steam_id
),
matches_agg AS (
    SELECT
        em.event_id,
        mlp.steam_id,
        COUNT(DISTINCT em.match_id)::int AS matches_played
    FROM e_matches em
    ...
    GROUP BY em.event_id, mlp.steam_id
)
SELECT
    m.event_id,
...
FROM matches_agg m
LEFT JOIN kd_agg kd
  ON kd.event_id = m.event_id AND kd.steam_id = m.steam_id
LEFT JOIN assists_agg a
  ON a.event_id = m.event_id AND a.steam_id = m.steam_id;
