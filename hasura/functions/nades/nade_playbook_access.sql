...
-- collection primitives in nade_access.sql, and deliberately built on the same
-- is_nade_team_member / is_nade_team_admin helpers so a team's book, its
...
CREATE OR REPLACE FUNCTION public.can_view_nade_playbook(
    playbook public.nade_playbooks,
    hasura_session json
) RETURNS boolean
LANGUAGE plpgsql
STABLE
...
BEGIN
    ...
    IF playbook.visibility = 'Team' THEN
        RETURN public.is_nade_team_member(playbook.team_id, _steam_id);
    END IF;
...
CREATE OR REPLACE FUNCTION public.can_edit_nade_playbook(
    playbook public.nade_playbooks,
    hasura_session json
) RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
    ...
    IF playbook.visibility = 'Team' THEN
        RETURN public.is_nade_team_admin(playbook.team_id, _steam_id);
    END IF;
