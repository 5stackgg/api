-- Visibility/edit access primitives for the nade library. Single file so the
...
CREATE OR REPLACE FUNCTION public.is_nade_team_member(
    _team_id uuid,
    _steam_id bigint
) RETURNS boolean
LANGUAGE sql
...
CREATE OR REPLACE FUNCTION public.is_nade_team_admin(
    _team_id uuid,
    _steam_id bigint
) RETURNS boolean
LANGUAGE sql
...
CREATE OR REPLACE FUNCTION public.can_view_nade_lineup(
...
BEGIN
    ...
    IF lineup.visibility = 'Team' THEN
        RETURN public.is_nade_team_member(lineup.team_id, _steam_id);
    END IF;
...
CREATE OR REPLACE FUNCTION public.can_edit_nade_lineup(
    lineup public.nade_lineups,
    hasura_session json
) RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
    ...
    IF lineup.visibility = 'Team' THEN
        RETURN public.is_nade_team_admin(lineup.team_id, _steam_id);
    END IF;
...
CREATE OR REPLACE FUNCTION public.can_view_nade_collection(
    collection public.nade_collections,
    hasura_session json
) RETURNS boolean
LANGUAGE plpgsql
STABLE
...
BEGIN
    ...
    IF collection.visibility = 'Team' THEN
        RETURN public.is_nade_team_member(collection.team_id, _steam_id);
    END IF;
...
CREATE OR REPLACE FUNCTION public.can_edit_nade_collection(
    collection public.nade_collections,
    hasura_session json
) RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
    ...
    IF collection.visibility = 'Team' THEN
        RETURN public.is_nade_team_admin(collection.team_id, _steam_id);
    END IF;
...
-- second round trip.
CREATE OR REPLACE FUNCTION public.nade_lineup_my_vote(
    lineup public.nade_lineups,
    hasura_session json
) RETURNS smallint
LANGUAGE plpgsql
...
BEGIN
    ...
    FROM public.nade_lineup_votes v
    WHERE v.nade_lineup_id = lineup.id AND v.steam_id = _steam_id;

    RETURN _vote;
END;
$$;

CREATE OR REPLACE FUNCTION public.nade_lineup_is_favorited(
    lineup public.nade_lineups,
    hasura_session json
) RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
    ...
    RETURN EXISTS (
        ...
        WHERE f.nade_lineup_id = lineup.id AND f.steam_id = _steam_id
    );
END;
$$;
