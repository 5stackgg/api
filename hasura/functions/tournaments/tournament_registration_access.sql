-- Who may register the team an invite was addressed to: exactly the people
-- public_tournament_teams.yaml already lets register one -- its owner, its
-- captain, or a team_roster Admin.
CREATE OR REPLACE FUNCTION public.player_may_register_team(
    _team_id uuid,
...
-- tbi_tournament_free_agents passes NULL, so an invited team gets to enter as a
-- team and its members do not each get a free-agent slot out of it.
--
...
AS $$
    ...
    OR EXISTS (
        ...
        WHERE u.tournament_id = _tournament_id
          ...
          AND public.player_may_register_team(u.team_id, _player_steam_id)
    );
$$;

-- The same question with no team in hand: may this player enter AT ALL, as
-- themselves or with any team that was let in? The gate on the tournament page
-- has no team to name yet, so asking the 3-argument form would tell a captain
-- who was invited by team that they still need an invite.
CREATE OR REPLACE FUNCTION public.tournament_registration_unlocked_any(
    _tournament_id uuid,
    _player_steam_id bigint
)
RETURNS boolean
...
STABLE
AS $$
    SELECT public.tournament_registration_unlocked(_tournament_id, _player_steam_id)
        OR EXISTS (
            SELECT 1
            FROM public.tournament_registration_unlocks u
            WHERE u.tournament_id = _tournament_id
...
-- without being handed anything it could redeem.
