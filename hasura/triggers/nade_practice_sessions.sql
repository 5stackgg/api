CREATE OR REPLACE FUNCTION public.tbiu_nade_practice_sessions() RETURNS TRIGGER
    LANGUAGE plpgsql
    AS $$
BEGIN
    NEW.updated_at = now();
...
DROP TRIGGER IF EXISTS tbiu_nade_practice_sessions ON public.nade_practice_sessions;
CREATE TRIGGER tbiu_nade_practice_sessions
    BEFORE INSERT OR UPDATE ON public.nade_practice_sessions
    FOR EACH ROW EXECUTE FUNCTION public.tbiu_nade_practice_sessions();
