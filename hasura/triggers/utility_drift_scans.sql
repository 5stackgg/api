CREATE OR REPLACE FUNCTION public.tbiu_utility_drift_scans() RETURNS TRIGGER
    LANGUAGE plpgsql
    AS $$
BEGIN
    NEW.updated_at = now();
...
DROP TRIGGER IF EXISTS tbiu_utility_drift_scans ON public.utility_drift_scans;
CREATE TRIGGER tbiu_utility_drift_scans
    BEFORE INSERT OR UPDATE ON public.utility_drift_scans
    FOR EACH ROW EXECUTE FUNCTION public.tbiu_utility_drift_scans();
